...
| **Generic** | `call_tool(name, args)` — escape hatch for any anan tool |
...
Dataset: [anan/terminal-bench-2](https://huggingface.co/datasets/anan/terminal-bench-2) on HuggingFace.
...
TBLite is a thin subclass of TerminalBench2 — only the dataset and timeouts differ. Created by the OpenThoughts Agent team (Snorkel AI + Bespoke Labs). Dataset: [anan/openthoughts-tblite](https://huggingface.co/datasets/anan/openthoughts-tblite).
...
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which anan toolsets to enable |
...
env:
  ...
  dataset_name: "anan/terminal-bench-2"
  tokenizer_name: "anan/Hermes-3-Llama-3.1-8B"
  use_wandb: true
...
- Python >= 3.11
- `atroposlib`: `pip install git+https://github.com/anan/atropos.git`
- An LLM API key (OpenRouter, OpenAI, or self-hosted VLLM/SGLang)

### For Modal-sandboxed benchmarks (TB2, TBLite)

- [Modal](https://modal.com) account and CLI: `pip install "anan[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "anan[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
