...
| `anan_cli/cron.py` | CLI `anan cron` subcommands |
...
Jobs are stored in `~/.anan/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `anan cron` commands are run or during active CLI sessions.
...
```python
# ~/.anan/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.anan/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `anan cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.
