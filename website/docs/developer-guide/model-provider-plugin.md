...
Model provider plugins declare an inference backend — an OpenAI-compatible endpoint, an Anthropic Messages server, a Codex-style Responses API, or a Bedrock-native surface — that anan Agent can route `AIAgent` calls through. Every built-in provider (OpenRouter, Anthropic, GMI, DeepSeek, Nvidia, …) ships as one of these plugins. Third parties can add their own by dropping a directory under `$ANAN_HOME/plugins/model-providers/` with zero changes to the repo.
...
1. **Bundled plugins** — `<repo>/plugins/model-providers/<name>/` — ship with anan Agent
2. **User plugins** — `$ANAN_HOME/plugins/model-providers/<name>/` — drop in any directory; no restart required for subsequent sessions
3. **Legacy single-file** — `<repo>/providers/<name>.py` — back-compat for out-of-tree editable installs

**User plugins override bundled plugins of the same name** because `register_provider()` is last-writer-wins. Drop a `$ANAN_HOME/plugins/model-providers/gmi/` directory to replace the built-in GMI profile without touching the repo.
...
Say you want to point `gmi` at your private staging endpoint for testing. Create `~/.anan/plugins/model-providers/gmi/__init__.py`:
...
Four values are recognized. anan Agent picks one based on:
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider" — if it's not `api_key`, the PluginManager still records the manifest but anan Agent's CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.
...
Point `ANAN_HOME` at a temp directory so you don't pollute your real config:

```bash
export ANAN_HOME=/tmp/anan-plugin-test
mkdir -p $ANAN_HOME/plugins/model-providers/my-provider
cat > $ANAN_HOME/plugins/model-providers/my-provider/__init__.py <<'EOF'
from providers import register_provider
from providers.base import ProviderProfile
register_provider(ProviderProfile(
    name="my-provider",
...
anan -z "hello" --provider my-provider -m some-model
```
...
Like any anan Agent plugin, model providers can ship as a pip package. Add an entry point to your `pyproject.toml`:

```toml
[project.entry-points."anan.plugins"]
...
See [Building a anan Agent Plugin](/docs/guides/build-a-anan-plugin#distribute-via-pip) for the full entry-points setup.
...
- [Building a anan Agent Plugin](/docs/guides/build-a-anan-plugin) — general plugin authoring
