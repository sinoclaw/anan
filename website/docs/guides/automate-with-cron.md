...
description: "Real-world automation patterns using anan Agent cron — monitoring, reports, pipelines, and multi-skill workflows"
---
...
```bash
mkdir -p ~/.anan/scripts
```

```python title="~/.anan/scripts/watch-site.py"
...
```bash
/cron add "every 1h" "If the script output says CHANGE DETECTED, summarize what changed on the page and why it might matter. If it says NO_CHANGE, respond with just [SILENT]." --script ~/.anan/scripts/watch-site.py --name "Pricing monitor" --deliver telegram
```
...
```python title="~/.anan/scripts/collect-prices.py"
import json, os, urllib.request
from datetime import datetime

DATA_DIR = os.path.expanduser("~/.anan/data/prices")
...
If there's a significant move, explain what happened." \
  --script ~/.anan/scripts/collect-prices.py \
  --name "Price tracker" \
  --deliver telegram
```
