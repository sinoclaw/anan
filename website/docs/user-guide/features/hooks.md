...
anan Agent has three hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.anan/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.anan/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
...
Each hook is a directory under `~/.anan/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".anan" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.anan/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.anan/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.anan/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.anan/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".anan" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.anan/hooks/session-webhook/HOOK.yaml
...
A popular pattern from the community: drop a Markdown checklist at `~/.anan/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."

This tutorial shows how to build it yourself as a user-defined hook. anan Agent does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.

#### What we're building

1. A file at `~/.anan/BOOT.md` with natural-language startup instructions.
...
Create `~/.anan/BOOT.md`. Write it as if you were giving instructions to a human assistant:
...
**`~/.anan/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.anan/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.anan/hooks/boot-md/handler.py`**

```python
"""Run ~/.anan/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".anan" / "BOOT.md"
...
```bash
anan logs --follow --level INFO | grep boot-md
```
...
Delete `~/.anan/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
An earlier version of anan Agent shipped this as a built-in hook and silently spawned an agent with bare defaults on every gateway boot. That surprised users with custom endpoints and made the feature invisible to users who didn't know it was running. Keeping it as a documented pattern — built by you, in your hooks directory — means you see exactly what it does and opt in by writing the files.

### How It Works

1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.anan/hooks/`
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is anan Agent's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.
...
def notify_approval(command, description, session_key, **kwargs):
    title = "anan Agent needs approval"
...
Declare shell-script hooks in your `cli-config.yaml` and anan Agent will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`anan_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.anan/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.anan/agent-hooks/` (by convention) | `~/.anan/plugins/<name>/` | `~/.anan/hooks/<name>/` |
...
Each time the event fires, anan Agent spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.anan/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.anan/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.anan/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.anan/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.anan/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.anan/agent-hooks/inject-cwd-context.sh
...
hooks:
  subagent_stop:
    - command: "~/.anan/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.anan/agent-hooks/log-orchestration.sh
log=~/.anan/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time anan Agent sees it, then persists the decision to `~/.anan/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.
