...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, anan Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.anan/config.yaml`:
...
- **CLI mode** — configured in `~/.anan/config.yaml`, loaded at startup
