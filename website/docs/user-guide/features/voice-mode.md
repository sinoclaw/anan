...
:::tip
The record key is configurable via `voice.record_key` in `~/.anan/config.yaml` (default: `ctrl+b`).
:::
...
:::tip
To disable the mention requirement in server channels, add to `~/.anan/.env`:
```bash
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.anan/.env`
...
- Review gateway logs: `tail -f ~/.anan/logs/gateway.log`
