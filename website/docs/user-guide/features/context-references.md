...
Type `@` followed by a reference to inject content directly into your message. anan Agent expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- anan Agent env: `$ANAN_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ANAN_HOME/skills/.hub/`
