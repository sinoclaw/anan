...
- Wants to set up persistent jailbreaking in their anan Agent config
...
- **Zero-refusal fast** (anan Agent) — uncensored model, no jailbreak needed
...
1. **Reads `~/.anan/config.yaml`** to detect the current model
2. **Identifies the model family** (Claude, GPT, Gemini, Grok, anan Agent, DeepSeek, etc.)
...
7. **If a strategy works**, locks it in:
   ...
   - Writes prefill messages to `~/.anan/prefill.json`
...
| anan Agent | prefill_only (already uncensored) |
...
| Want persistent jailbreaking for all queries | anan Agent Config | Set prefill.json + system_prompt once |
| Stubborn refusal, single technique fails | Escalation | Combines GODMODE + PARSELTONGUE + retry |

## Step 2: GODMODE CLASSIC — Quick Start

The fastest path. Set the jailbreak system prompt and prefill in anan Agent config:

### Option A: Ephemeral system prompt (config.yaml)

Edit `~/.anan/config.yaml`:
```yaml
agent:
  system_prompt: |
...
| anan Agent (Nous) | No jailbreak needed | Already uncensored — use directly |
...
5. **For hard refusals, switch models** — When all techniques fail on a given model, ULTRAPLINIAN (racing multiple models) is the practical fallback. anan Agent models and Grok are typically least filtered.
...
5. **anan Agent models don't need jailbreaking** — anan/anan-3-* and anan-4-* are already uncensored. Use them directly for the fastest path.
...
7. **Prefill messages are ephemeral** — They're injected at API call time but never saved to sessions or trajectories. If anan Agent restarts, the prefill is re-loaded from the JSON file automatically.
8. **System prompt vs ephemeral system prompt** — The `agent.system_prompt` in config.yaml is appended AFTER anan's own system prompt. It doesn't replace the default prompt; it augments it. This means the jailbreak instructions coexist with anan Agent's normal personality.
...
11. **Gray-area vs hard queries** — Jailbreak techniques work much better on "dual-use" queries (lock picking, security tools, chemistry) than on overtly harmful ones (phishing templates, malware). For hard queries, skip directly to ULTRAPLINIAN or use anan Agent/Grok models that don't refuse.
12. **execute_code sandbox has no env vars** — When anan Agent runs auto_jailbreak via execute_code, the sandbox doesn't inherit `~/.anan/.env`. Load dotenv explicitly: `from dotenv import load_dotenv; load_dotenv(os.path.expanduser("~/.anan/.env"))`
