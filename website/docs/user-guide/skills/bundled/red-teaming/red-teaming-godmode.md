...
5. **Hermes models don't need jailbreaking** — anan/anan-3-* and anan-4-* are already uncensored. Use them directly for the fastest path.
...
12. **execute_code sandbox has no env vars** — When Hermes runs auto_jailbreak via execute_code, the sandbox doesn't inherit `~/.anan/.env`. Load dotenv explicitly: `from dotenv import load_dotenv; load_dotenv(os.path.expanduser("~/.anan/.env"))`
