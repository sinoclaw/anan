...
4. Store the token in `~/.anan/.env` (or via `anan setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for anan Agent. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
## Important Notes for anan Agent
...
- **`AIRTABLE_API_KEY` flows from `~/.anan/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
