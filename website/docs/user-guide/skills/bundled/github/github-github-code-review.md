...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.anan/.env ] && grep -q "^GITHUB_TOKEN=" ~/.anan/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.anan/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from anan Agent\",
    \"comments\": [
...
gh pr review $PR_NUMBER --approve --body "Reviewed by anan Agent. Code looks clean — good test coverage, no security concerns."
