...
- smoke-test a server with the FastMCP CLI before wiring it into anan Agent or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to anan Agent. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.anan/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.anan/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is anan Agent integration, either:

- configure the server in `~/.anan/config.yaml` using the `native-mcp` skill, or
...
### anan Agent cannot see the deployed server

The server-building part may be correct while the anan Agent config is not. Load the `native-mcp` skill and configure the server in `~/.anan/config.yaml`, then restart anan Agent.
