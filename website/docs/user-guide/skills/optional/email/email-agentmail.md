...
Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. anan@agentmail.to).

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `anan skills install official/email/agentmail` |
...
Add to `~/.anan/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `anan`)
   - The agent gets address: `anan@agentmail.to`
2. Send an email:
...
```
1. create_inbox (username: "anan-outreach")
