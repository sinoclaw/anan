...
description: "Configure and use Honcho memory with anan Agent -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with anan Agent -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
...
# Honcho Memory for anan Agent

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every anan Agent profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/anan#running-honcho-locally-with-anan
...
3. **AI peer card** -- the identity card for this anan Agent profile's AI peer
...
Honcho models conversations as interactions between **peers**. anan Agent creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this anan Agent instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each anan Agent run |
...
Each anan Agent profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
Guidelines for anan Agent when Honcho memory is active.
...
Config file: `$ANAN_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `anan honcho setup`. Ensure `memory.provider: honcho` is in `~/.anan/config.yaml`.
...
| Command | Description |
|---------|-------------|
| `anan honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `anan honcho status` | Show resolved config, connection test, peer info for active profile |
| `anan honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `anan honcho disable` | Disable Honcho for the active profile |
| `anan honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `anan honcho peers` | Show peer identities across all profiles |
| `anan honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `anan honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
...
| `anan honcho sync` | Create host blocks for all anan Agent profiles that don't have one yet |
| `anan honcho migrate` | Step-by-step migration guide from OpenClaw native memory to anan Agent + Honcho |
