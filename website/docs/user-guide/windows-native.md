...
anan Agent runs natively on Windows 10 and Windows 11 — no WSL, no Cygwin, no Docker. This page is the deep dive: what works natively, what's WSL-only, what the installer actually does, and the Windows-specific knobs you might need to touch.
...
If you prefer a real POSIX environment (for the dashboard's embedded terminal, `fork` semantics, Linux-style file watchers, etc.), see the **[Windows (WSL2) Guide](./windows-wsl-quickstart.md)**. Both coexist cleanly: native data lives under `%LOCALAPPDATA%\anan`, WSL data lives under `~/.anan`.
:::
...
No admin rights required. The installer goes to `%LOCALAPPDATA%\anan\` and adds `anan` to your **User PATH** — open a new terminal after it finishes.
...
| `-anan AgentHome` | `%LOCALAPPDATA%\anan` | Override data directory |
| `-InstallDir` | `%LOCALAPPDATA%\anan\anan` | Override code location |
...
3. **Installs Node.js 22** (winget if available, else a portable Node tarball unpacked under `%LOCALAPPDATA%\anan\node`). Used for the browser tool and the WhatsApp bridge.
4. **Installs portable Git** — if `git` is already on PATH the installer uses it; otherwise it downloads a trimmed, self-contained **PortableGit** (~45 MB, from the official `git-for-windows` release) to `%LOCALAPPDATA%\anan\git`. No admin, no Windows installer registry, no interference with anything else on the box.
5. **Clones the repo** to `%LOCALAPPDATA%\anan\anan` and creates a virtualenv inside it.
...
8. **Sets `SINOCLAW_GIT_BASH_PATH`** to the resolved `bash.exe` so anan Agent finds it deterministically in fresh shells.
9. **Adds `%LOCALAPPDATA%\anan\bin` to User PATH** — exposes the `anan` command after you open a new terminal.
...
## How anan Agent runs shell commands on Windows

anan Agent's terminal tool runs commands through **Git Bash**, same strategy Claude Code uses. This sidesteps the POSIX-vs-Windows gap without rewriting every tool.

Resolution order for `bash.exe`:

1. `SINOCLAW_GIT_BASH_PATH` environment variable if set.
2. `%LOCALAPPDATA%\anan\git\usr\bin\bash.exe` (installer-managed PortableGit).
3. `%LOCALAPPDATA%\anan\git\bin\bash.exe` (older Git-for-Windows layout).
...
The installer sets `SINOCLAW_GIT_BASH_PATH` explicitly so fresh PowerShell sessions don't have to re-discover. Override it if you want anan Agent to use a specific bash — for example, your system Git Bash or a WSL-hosted bash via a symlink.

**Pitfall:** MinGit's layout is different from the full Git-for-Windows installer — bash lives under `usr\bin\bash.exe`, not `bin\bash.exe`. anan Agent checks both. If you're manually unpacking a MinGit zip, make sure you pick the **non-busybox** variant (`MinGit-*-64-bit.zip`, not `MinGit-*-busybox*.zip`) — busybox builds ship `ash` instead of `bash` and most coreutils are missing.

## UTF-8 console on Windows

Python's default stdio on Windows uses the console's active code page (usually cp1252 or cp437). anan Agent's banner, slash-command list, tool feed, Rich panels, and skill descriptions all contain Unicode. Without intervention, any of that crashes with `UnicodeEncodeError: 'charmap' codec can't encode character…`.

The fix is in `anan_cli/stdio.py::configure_windows_stdio()`, called early in every entry point (`cli.py::main`, `anan_cli/main.py::main`, `gateway/run.py::main`). It:
...
anan Agent's Windows stdio shim now sets `EDITOR=notepad` as a default. Notepad ships with every Windows install and works as a blocking editor — `subprocess.call(["notepad", file])` blocks until the window closes.
...
The `--wait` flag on VS Code is critical — without it the editor returns immediately and anan Agent gets a blank buffer back.
...
1. `schtasks /Create /SC ONLOGON /RL LIMITED /TN anan AgentGateway` — registers a task that runs at your login with standard (non-elevated) permissions. No UAC prompt.
...
Services require admin rights to install and tie the gateway's lifecycle to machine boot, not user login. The typical anan Agent user wants: log in → gateway available, log out → gateway gone. Scheduled Tasks do exactly that without elevation. If you genuinely want a service, use `nssm` or `sc create` manually — but you probably don't.

## Data layout

| Path | Contents |
|---|---|
| `%LOCALAPPDATA%\anan\anan\` | Git checkout + venv. Safe to `Remove-Item -Recurse` and reinstall. |
| `%LOCALAPPDATA%\anan\git\` | PortableGit (only if the installer provisioned it). |
| `%LOCALAPPDATA%\anan\node\` | Portable Node.js (only if the installer provisioned it). |
| `%LOCALAPPDATA%\anan\bin\` | `anan.cmd` shim, added to User PATH. |
| `%USERPROFILE%\.anan\` | Your config, auth, skills, sessions, logs. **Survives reinstalls.** |

The split is deliberate: `%LOCALAPPDATA%\anan` is disposable infrastructure (you can blow it away and the one-liner restores it). `%USERPROFILE%\.anan` is your data — config, memory, skills, session history — and is identical in shape to a Linux install. Mirror it between machines and your anan Agent moves with you.

**Override `ANAN_HOME`:** set the environment variable to point at a different data dir. Works the same as on Linux.
...
- `shutil.which("agent-browser", path=...)` picks up the `.cmd` shim automatically — `CreateProcessW` can't execute an extensionless shebang, so anan Agent always resolves to the `.CMD` wrapper. Don't manually invoke the shebang script; always go through the `.cmd`.
...
## Running anan Agent on Windows — practical notes

### PATH after install

The installer adds `%LOCALAPPDATA%\anan\bin` to your **User PATH** via `[Environment]::SetEnvironmentVariable`. Existing terminals don't pick this up — open a new PowerShell window (or Windows Terminal tab) after installation. Close-and-reopen, don't `$env:PATH += …` by hand unless you know what you're doing.

Verify:

```powershell
Get-Command anan        # should print C:\Users\<you>\AppData\Local\anan\bin\anan.cmd
anan --version
```

### Environment variables

anan Agent honors both `$env:X` (process-scope) and User environment variables (permanent, set in System Properties → Environment Variables). Setting API keys in `%USERPROFILE%\.anan\.env` is the normal path — same as Linux:
...
| `EDITOR` / `VISUAL` | Your editor for `/edit` and `Ctrl-X Ctrl-E`. anan Agent defaults to `notepad` if both are unset. |
...
That's the clean path — removes the schtasks entry, Startup folder shortcut, `anan.cmd` shim, deletes `%LOCALAPPDATA%\anan\anan\`, and trims the User PATH. It leaves `%USERPROFILE%\.anan\` alone (your config, auth, skills, sessions, logs) in case you're reinstalling.

To nuke everything:

```powershell
anan uninstall
Remove-Item -Recurse -Force "$env:USERPROFILE\.anan"
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\anan"
```
...
Consequence: any codepath that said "check if this PID is alive" via `os.kill(pid, 0)` on Windows was silently killing the target. anan Agent migrated every such site (14 across 11 files) to `gateway.status._pid_exists()`, which uses `psutil.pid_exists()` (which in turn uses `OpenProcess + GetExitCodeProcess` on Windows — no signals). If you're writing a plugin or patch, use `psutil.pid_exists()` directly or `gateway.status._pid_exists()` — never `os.kill(pid, 0)`.
...
**`anan: command not found` right after install.**
Open a new PowerShell window. The installer added `%LOCALAPPDATA%\anan\bin` to User PATH, but existing shells need to be restarted to pick it up. In the meantime you can run `& "$env:LOCALAPPDATA\anan\bin\anan.cmd"`.

**`WinError 193: %1 is not a valid Win32 application` when running a tool.**
You hit a shebang-script invocation that bypassed the `.cmd` shim. anan Agent resolves commands through `shutil.which(cmd, path=local_bin)` so PATHEXT picks up `.CMD` — if you're invoking the tool via a hardcoded path instead, switch to the `.cmd` variant (e.g., `npx.cmd`, not `npx`).
...
The installer provisions Node 22 at `%LOCALAPPDATA%\anan\node` but your PATH may have an older system Node 18 first. Either move anan Agent's node dir earlier on PATH, or delete the system install if you don't use Node elsewhere.
...
This is unrelated to Windows but sometimes surfaces first there. Usually it means your file path contains unescaped backslashes in a JSON body. Telegram should be receiving paths anan Agent normalizes, not raw Windows paths — if you're seeing this inside a custom plugin, make sure you're passing the anan-provided path, not `str(Path(...))` from user input.

**"Works on my other machine" encoding weirdness after `git pull`.**
If you edited anan Agent config or a skill on Windows using a non-UTF-8 editor (Notepad on older Windows versions, some Chinese IMEs), the file may have been saved with a BOM. anan Agent tolerates `utf-8-sig` on most config reads, but a BOM inside a folded YAML scalar (`description: >`) silently breaks YAML parsing. Re-save the file as plain UTF-8 without BOM.
...
- **[CLI Reference](../reference/cli-commands.md)** — every `anan` subcommand.
