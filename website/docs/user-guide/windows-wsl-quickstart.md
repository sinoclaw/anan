...
- Interactive chat, gateway (Telegram/Discord/etc.), cron scheduler, browser tool, MCP servers, and most anan Agent features all run natively on Windows.
...
This guide covers the parts of that split that specifically affect anan Agent: installing WSL2, getting files back and forth between Windows and Linux, networking in both directions, and the pitfalls people actually hit.
...
The native Windows install runs in Windows directly: your Windows terminal (PowerShell, Windows Terminal, etc.), Windows filesystem paths (`C:\Users\…`), and Windows processes.  anan Agent uses Git Bash to run shell commands, which is how Claude Code and other agents handle Windows today — it sidesteps the POSIX-vs-Windows gap without a full rewrite.

WSL2 runs a real Linux kernel in a lightweight VM, so anan Agent inside it is essentially identical to running on Ubuntu.  That's valuable when you want a real POSIX environment: `fork`, `/tmp`, UNIX sockets, signal semantics, PTY-backed terminals, shells like `bash`/`zsh`, and tools like `rg`, `git`, `ffmpeg` that behave the way they do on Linux.
...
anan Agent does not work reliably on WSL1 — WSL1 translates Linux syscalls on the fly and some behaviors (procfs, signals, network) diverge from real Linux.
...
### Where to put anan Agent and your projects

**Rule of thumb: keep everything Linux-ish inside the Linux filesystem.**

- Your anan Agent install (`~/.anan/`) — Linux side. The installer already does this.
...
Clone inside WSL. Always, unless you have a specific reason not to. A typical anan Agent workflow (`anan chat`, tool calls that `rg`/`ripgrep` the repo, file watchers, background gateway) will be dramatically faster and more reliable against `~/code/myrepo` than `/mnt/c/Users/you/myrepo`.

One exception: **MCP bridges that launch Windows binaries.** If you're using `chrome-devtools-mcp` through `cmd.exe` (see [MCP guide: WSL → Windows Chrome](/docs/guides/use-mcp-with-anan#wsl2-bridge-anan-in-wsl-to-windows-chrome)), Windows may complain with a `UNC` warning if anan Agent's current working directory is `~`. In that case, start anan Agent from somewhere under `/mnt/c/` so the Windows process has a drive-letter cwd.
...
### Case 1 — anan Agent in WSL talks to a service on Windows

Most common: you're running **Ollama, LM Studio, or a llama-server on Windows**, and anan Agent (inside WSL) needs to hit it.
...
### Case 2 — Something on Windows (or your LAN) talks to anan Agent in WSL

This is the reverse direction and is less documented elsewhere, but it's what you need for:

- Using the anan Agent **web dashboard** from a Windows browser.
...
On **NAT mode** (Windows 10 / older Windows 11), the default "localhost forwarding" in WSL2 will generally forward Linux-side `127.0.0.1` binds to Windows `localhost`, so a anan Agent service started with `--host 127.0.0.1` is usually reachable as `http://localhost:PORT` from Windows. If it isn't:
...
2. **Port-forward Windows → WSL VM.** In mirrored mode this is automatic. In NAT mode you have to do it yourself, per port, in Admin PowerShell:
   ...
   New-NetFirewallRule -DisplayName "anan Agent WSL 8080" `
...
## Running anan Agent services long-term on Windows

The anan Agent [Tool Gateway](/docs/user-guide/features/tool-gateway) and the API server are long-lived processes. In WSL2 you have a few options for keeping them up.
...
AMD ROCm and Intel Arc support inside WSL2 is still evolving and outside anan Agent's test matrix — it may work with current drivers but we don't have a recipe to recommend.
...
anan Agent's cwd is inside the Linux filesystem, and Windows `cmd.exe` doesn't know what to do with it. Start anan Agent from `/mnt/c/...` for that session, or use a wrapper that `cd`s to a Windows-reachable path before invoking the Windows executable.
...
**`anan` not found after running the installer.**
...
- **[MCP guide → WSL → Windows Chrome](/docs/guides/use-mcp-with-anan#wsl2-bridge-anan-in-wsl-to-windows-chrome)** — controlling your signed-in Windows Chrome from anan Agent in WSL.
