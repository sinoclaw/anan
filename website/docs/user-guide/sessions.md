...
anan Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.anan/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.anan/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`anan` or `anan chat`) |
...
anan -c "my project"   # → resumes "my project #3"
```
...
anan --resume 20250305_091523_a1b2c3d4
anan -r 20250305_091523_a1b2c3d4

# Resume by title
anan --resume "refactoring auth"

# Or with the chat subcommand
anan chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `anan sessions list`.

### Conversation Recap on Resume

When you resume a session, anan Agent displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a anan session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.anan/config.yaml`:
...
anan Agent automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `anan sessions list` or `anan sessions browse`.
...
```bash
anan sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), anan Agent creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`anan -c "my project"`), it automatically picks the most recent session in the lineage.
...
anan Agent provides a full set of session management commands via `anan sessions`:
...
# Filter by platform
anan sessions list --source telegram

# Show more sessions
anan sessions list --limit 50
```
...
anan sessions export backup.jsonl

# Export sessions from a specific platform
anan sessions export telegram-history.jsonl --source telegram

# Export a single session
anan sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
anan sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
anan sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
anan sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
anan sessions prune --older-than 30

# Only prune sessions from a specific platform
anan sessions prune --source telegram --older-than 60

# Skip confirmation
anan sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`anan insights`](/docs/reference/cli-commands#anan-insights).
...
When anan Agent cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, anan Agent uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to anan Agent in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.anan/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.anan/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.anan/sessions/sessions.json` | Maps session keys to active session IDs |
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every anan Agent process in the same `ANAN_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.anan/config.yaml`:
...
anan sessions delete <session_id>
