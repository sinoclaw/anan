...
The approval system supports three modes, configured via `approvals.mode` in `~/.anan/config.yaml`:
...
Configure the timeout in `~/.anan/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.anan/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.anan/.env` | Overwrite sensitive file via redirection |
...
Commands approved with "always" are saved to `~/.anan/config.yaml`:
...
Set allowed user IDs as comma-separated values in `~/.anan/.env`:
...
Container resources are configurable in `~/.anan/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.anan/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `ANAN_HOME`. Skills declare these in frontmatter:
...
Paths are relative to `~/.anan/`. Files are mounted to `/root/.anan/` inside the container.
...
4. **Store secrets securely** — keep API keys in `~/.anan/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.anan/logs/` for unauthorized access attempts
10. **Keep updated** — run `anan update` regularly for security patches
