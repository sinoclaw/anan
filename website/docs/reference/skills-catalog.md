...
Hermes ships with a large built-in skill library copied into `~/.anan/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.

Hermes also syncs bundled skills on `anan update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.anan/skills/` tree, it is still shipped with Hermes; restore it with `anan skills reset <name> --restore`.
...
| Skill | Description | Path |
|-------|-------------|------|
| [`debugging-anan-tui-commands`](/docs/user-guide/skills/bundled/software-development/software-development-debugging-anan-tui-commands) | Debug Hermes TUI slash commands: Python, gateway, Ink UI. | `software-development/debugging-anan-tui-commands` |
