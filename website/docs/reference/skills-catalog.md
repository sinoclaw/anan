...
Hermes also syncs bundled skills on `anan update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.anan/skills/` tree, it is still shipped with Hermes; restore it with `sinoclaw skills reset <name> --restore`.
...
| [`anan`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-anan) | Configure, extend, or contribute to Sinoclaw Agent. | `autonomous-ai-agents/anan` |
