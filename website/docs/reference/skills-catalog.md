...
anan Agent ships with a large built-in skill library copied into `~/.anan/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.

anan Agent also syncs bundled skills on `anan update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.anan/skills/` tree, it is still shipped with anan Agent; restore it with `anan skills reset <name> --restore`.
...
| [`anan`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-anan) | Configure, extend, or contribute to anan Agent. | `autonomous-ai-agents/anan` |
...
| [`kanban-worker`](/docs/user-guide/skills/bundled/devops/devops-kanban-worker) | Pitfalls, examples, and edge cases for anan Agent Kanban workers. The lifecycle itself is auto-injected into every worker's system prompt as KANBAN_GUIDANCE (from agent/prompt_builder.py); this skill is what you load when you want deeper det... | `devops/kanban-worker` |
...
| Skill | Description | Path |
|-------|-------------|------|
| [`debugging-anan-tui-commands`](/docs/user-guide/skills/bundled/software-development/software-development-debugging-anan-tui-commands) | Debug anan Agent TUI slash commands: Python, gateway, Ink UI. | `software-development/debugging-anan-tui-commands` |
| [`anan-skill-authoring`](/docs/user-guide/skills/bundled/software-development/software-development-anan-skill-authoring) | Author in-repo SKILL.md: frontmatter, validator, structure. | `software-development/anan-skill-authoring` |
...
| [`plan`](/docs/user-guide/skills/bundled/software-development/software-development-plan) | Plan mode: write markdown plan to .anan/plans/, no exec. | `software-development/plan` |
