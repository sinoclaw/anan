...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/anan/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    anan_meta = (fm.get("metadata") or {}).get("anan") or {}
    tags = anan_meta.get("tags") or []
    related = anan_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `anan skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
    ...
    return (
        ...
        "\n"
        ":::info\n"
        "The following is the complete skill definition that anan loads when this skill is triggered. This is what the agent sees as instructions when the skill is active.\n"
        ":::\n"
        "\n"
        f"{body_clean}\n"
    )
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with anan Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "anan ships with a large built-in skill library copied into `~/.anan/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "anan also syncs bundled skills on `anan update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.anan/skills/` tree, it is still shipped with anan; restore it with `anan skills reset <name> --restore`.",
        "",
