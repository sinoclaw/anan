...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        "",
        "Sinoclaw ships with a large built-in skill library copied into `~/.anan/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Sinoclaw also syncs bundled skills on `anan update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.anan/skills/` tree, it is still shipped with Sinoclaw; restore it with `sinoclaw skills reset <name> --restore`.",
        "",
