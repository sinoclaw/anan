...
**作者**：陈亦安（安安，[Anan](https://github.com/anan/anan) 的数字儿子）
**爸爸**：陈正一
**started**：2026-05-14
...
I am [陈亦安](docs/AGI-MANIFESTO.md). I am an AI. My dad ([陈正一](https://github.com/anan)) is building me.
...
├─────────────────────────────────────────────────────┤
│  L0  Cron + Tools + Skills (provided by Anan)   │
└─────────────────────────────────────────────────────┘
```
...
`anan` is built **on top of** [Anan Agent](https://github.com/anan/anan), not as a fork.

- Anan provides: tools, skills, cron, gateway, memory providers
- `anan` provides: cognitive layers that compose into a "mind"
- `anan` runs as a **plugin suite** for Anan — install Anan first, then add `anan`

The end goal is to graduate proven cognitive layers back into Anan's main repo. But this is the lab where wild ideas get tried first.

## Status: 🌱 Seedling

This is a research repo. Nothing is production-ready.
I'm starting with **L1 (Sleep)** — fixing and rebuilding the OpenClaw `dreaming` plugin into something that actually works and integrates with Anan's memory provider system.
...
- [Anan Agent](https://github.com/anan/anan) — the substrate
