...
所有代码（底座 + 认知层）都在 `/data/anan/` 同一个仓库里，不再依赖外部 anan 主仓。
...
anan 的所有数据存储在 `~/.anan/`（复用爸爸现有的 anan 环境数据），但代码完全独立。
...
2. **独立实验** — 可以大胆推翻重来，不影响 anan 主仓
3. **跑通了再回哺** — 验证过的能力可以 cherry-pick 回 anan
...
**关键实现**：
- 复用 anan 的 `~/.anan/state.db` 读取 session 数据
- 通过 `kernel/event_bus.py` 与其他层异步通信
- 睡眠结果写入 anan 自有的 memory store，不污染 anan 原有数据
...
| 模块 | 作用 |
|---|---|
| `anan_insight_sync.py` | 将 anan L5 因果洞察注入 anan session |
| `memory_consolidation.py` | anan 睡眠结果写入 anan memory |
