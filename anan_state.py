#!/usr/bin/env python3
"""
SQLite State Store for Anan Agent.
...
DEFAULT_DB_PATH = get_anan_home() / "state.db"
...
# anan_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``anan_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see anan_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple anan processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None):
        ...
        try:
            ...
            # ``anan_state._set_last_init_error(None)`` explicitly.
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Anan, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Anan session.

        A Anan session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Anan session is already bound to any Telegram DM topic.
