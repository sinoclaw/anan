"""Windows UTF-8 bootstrap for Anan entry points.
...
should be imported at the very top of every Anan entry point
(``hermes``, ``anan``, ``sinoclaw-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import anan_bootstrap``
# (or ``from anan_bootstrap import apply_windows_utf8_bootstrap``) at
