...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - anan_home (str): The active ANAN_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.anan``.
    ...
    def save_config(self, values: Dict[str, Any], anan_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'hermes memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``anan_home`` is the active ANAN_HOME directory path.
