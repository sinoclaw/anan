...
from anan_constants import get_anan_home, get_skills_dir, is_wsl
...
_SINOCLAW_MD_NAMES = (".anan.md", "ANAN.md")


def _find_anan_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.anan.md`` or ``ANAN.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are anan Agent, an intelligent AI assistant created by anan Team. "
...
SINOCLAW_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Anan Agent "
    "itself, load the `anan` skill with skill_view(name='anan') "
    "before answering. Docs: https://anan.nousresearch.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.anan/kanban.db`. Your task id is in "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `anan kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Anan WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Anan itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Anan runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Anan itself is running. The host OS, home, and cwd "
                f"of the Anan process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Anan "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_anan_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.anan/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Anan Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `anan` skill "
            "first. It has the actual commands (e.g. `anan config set …`, `anan tools`, "
            "`anan setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: anan setup, anan setup tools, anan setup terminal, anan status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from ANAN_HOME and return its content, or None.
    ...
    try:
        ...
        logger.debug("Could not ensure ANAN_HOME before loading SOUL.md: %s", e)

    soul_path = get_anan_home() / "SOUL.md"
...
def _load_anan_md(cwd_path: Path) -> str:
    """.anan.md / ANAN.md — walk to git root."""
    anan_md_path = _find_anan_md(cwd_path)
    if not anan_md_path:
        return ""
    try:
        content = anan_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = anan_md_path.name
        try:
            rel = str(anan_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".anan.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", anan_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .anan.md / ANAN.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ANAN_HOME is independent and always included when present.
    ...
    # SOUL.md from ANAN_HOME only — skip when already loaded as identity
    if not skip_soul:
