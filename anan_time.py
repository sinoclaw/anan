...
Resolution order:
  ...
  2. ``timezone`` key in ``~/.anan/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Anan never
...
from anan_constants import get_config_path
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — Anan requires 3.9+)
