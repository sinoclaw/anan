"""Centralized logging setup for Anan Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.anan/logs/`` (profile-aware via ``get_anan_home()``).
...
from anan_constants import get_config_path, get_anan_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_anan_session_injector", False):
    ...
    _session_record_factory._anan_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``anan logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
...
def setup_logging(
    *,
    ...
    """Configure the Anan logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    anan_home
        Override for the Anan home directory.  Falls back to
        ``get_anan_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_anan_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._anan_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the anan group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from anan_cli.config import is_managed
