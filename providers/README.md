...
Registry and ABC for every inference provider Anan knows about.
...
`$ANAN_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
- `anan_cli/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `anan_cli/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `anan_cli/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `anan_cli/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `anan_cli/runtime_provider.py` reads `profile.api_mode` as a fallback
...
under `$ANAN_HOME/plugins/model-providers/` for a private plugin).
