...
Maps each role archetype to the Anan skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public anan
...
## Anan skills relevant to video production

### Visual / rendering skills (`anan/skills/creative/`)
...
### Design / pre-production skills (`anan/skills/creative/`)
...
### Audio / media skills (`anan/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`anan/skills/devops/`)
...
These are **not** Anan skills but external CLIs / APIs that profiles invoke.
...
## Built-in Anan tools for media review

These are native Anan tools — not invoked via terminal but through their own
...
key is present in `~/.anan/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Anan profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.anan/.env` → environment variable.
