...
- smoke-test a server with the FastMCP CLI before wiring it into Anan or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Anan. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
When the goal is Anan integration, either:
...
### Anan cannot see the deployed server

The server-building part may be correct while the Anan config is not. Load the `native-mcp` skill and configure the server in `~/.anan/config.yaml`, then restart Anan.
