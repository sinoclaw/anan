#!/usr/bin/env python3
"""
Anan CLI - Main entry point.

Usage:
    anan                     # Interactive chat (default)
    anan chat                # Interactive chat
    anan gateway             # Run gateway in foreground
    anan gateway start       # Start gateway as service
    anan gateway stop        # Stop gateway service
    anan gateway status      # Show gateway status
    anan gateway install     # Install gateway service
    anan gateway uninstall   # Uninstall gateway service
    anan setup               # Interactive setup wizard
    anan logout              # Clear stored authentication
    anan status              # Show status of all components
    anan cron                # Manage cron jobs
    anan cron list           # List cron jobs
    anan cron status         # Check if cron scheduler is running
    anan doctor              # Check configuration and dependencies
    anan honcho setup                    # Configure Honcho AI memory integration
    anan honcho status                   # Show Honcho config and connection status
    anan honcho sessions                 # List directory → session name mappings
    anan honcho map <name>               # Map current directory to a session name
    anan honcho peer                     # Show peer names and dialectic settings
    anan honcho peer --user NAME         # Set user peer name
    anan honcho peer --ai NAME           # Set AI peer name
    anan honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    anan honcho mode                     # Show current memory mode
    anan honcho mode [hybrid|honcho|local]  # Set memory mode
    anan honcho tokens                   # Show token budget settings
    anan honcho tokens --context N       # Set session.context() token cap
    anan honcho tokens --dialectic N     # Set dialectic result char cap
    anan honcho identity                 # Show AI peer identity representation
    anan honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    anan honcho migrate                  # Step-by-step migration guide: OpenClaw native → Anan + Honcho
    anan version             Show version
    anan update              Update to latest version
    anan uninstall           Uninstall Anan Agent
    anan acp                 Run as an ACP server for editor integration
    anan sessions browse     Interactive session picker with search

    anan claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: anan_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``anan_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``anan update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``anan_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, anan crashes on import and the user can't run
# ``anan update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (anan tools, anan setup, anan model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'anan {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any anan module import.
#
# Many modules cache ANAN_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ANAN_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.anan/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ANAN_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors anan_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ANAN_HOME is already set and no explicit flag was given, trust it.
    # This lets child processes (relaunch, subprocess) inherit the parent's
    # profile choice without having to pass --profile again.
    if profile_name is None and os.environ.get("ANAN_HOME"):
        return

    # 2. If no flag, check active_profile in the anan root
    if profile_name is None:
        try:
            from anan_constants import get_default_anan_root

            active_path = get_default_anan_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ANAN_HOME
    if profile_name is not None:
        try:
            from anan_cli.profiles import resolve_profile_env

            anan_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent anan from starting
            print(
...
load_anan_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → SINOCLAW_REDACT_SECRETS env
# var BEFORE anan_logging imports agent.redact (which snapshots the flag at
...
try:
    if "SINOCLAW_REDACT_SECRETS" not in os.environ:
        ...
        _cfg_path = get_anan_home() / "config.yaml"
        if _cfg_path.exists():
...
# Initialize centralized file logging early — all `anan` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from anan_logging import setup_logging as _setup_logging
...
try:
    from anan_cli.config import load_config as _load_config_early
    from anan_constants import apply_ipv4_preference as _apply_ipv4
...
from anan_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from anan_cli.config import get_env_path, get_anan_home, load_config
    from anan_cli.auth import get_auth_status

    # Determine whether Anan itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_anan_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_anan_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Anan has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Anan to use their tokens.
    if _has_anan_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, anan_bin
        cli_args: the original CLI arguments (everything after 'anan')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    anan_bin = container_info["anan_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo anan {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo anan {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  anan --tui --resume {target}")
    if title:
        print(f'  anan --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @anan/ink is missing or node_modules is behind package-lock.json.
    ...
    """
    ink = root / "node_modules" / "@anan" / "ink" / "package.json"
    if not ink.is_file():
        return True
...
def _tui_build_needed(tui_dir: Path) -> bool:
    if _anan_ink_bundle_stale(tui_dir):
        return True
...
def _anan_ink_bundle_stale(tui_dir: Path) -> bool:
    ink_root = tui_dir / "packages" / "anan-ink"
...
def _ensure_tui_node() -> None:
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ANAN_HOME": anan_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(anan_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    if tui_dev:
        if _anan_ink_bundle_stale(tui_dir):
            result = subprocess.run(
                [npm, "run", "build", "--prefix", "packages/anan-ink"],
                cwd=str(tui_dir),
                capture_output=True,
                text=True,
            )
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from anan_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="anan-tui-active-session-", suffix=".json"
    )
...
def _pin_kanban_board_env() -> None:
    ...
    (``anan kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``anan kanban boards switch`` from another session can flip the file
    ...
    if os.environ.get("SINOCLAW_KANBAN_BOARD"):
        return
    try:
        from anan_cli.kanban_db import get_current_board
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'anan sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Anan isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  anan setup")
        print()

        from anan_cli.setup import (
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'anan setup' at any time to configure.")
        sys.exit(1)
    ...
    # user's ~/.anan/config.yaml and return built-in defaults. Set BEFORE
...
def cmd_gateway(args):
    ...
    from anan_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from anan_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Anan?")
        print()
    ...
    session_dir = get_anan_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: anan gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  anan gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Anan Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  anan gateway")
        ...
        print()
        print("  Or install as a service: anan gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'anan whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from anan_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``anan model``) and the setup wizard
    ...
    from anan_cli.providers import resolve_provider_full
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'anan model' for "
                "available providers, or run 'anan doctor' to diagnose config "
                "issues."
            )
    ...
    # a leftover OPENAI_BASE_URL in ~/.anan/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.anan/.env if the active provider is not 'custom'.
    ...
    """
    from anan_cli.config import get_env_value, save_env_value, load_config
...
#
# Anan uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `anan model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `anan model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Anan only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``anan model`` flow, then route aux tasks to them here.
    """
    from anan_cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from anan_cli.auth import _prompt_model_selection
    from anan_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from anan_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from anan_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from anan_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from anan_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from anan_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from anan_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `anan auth` stores device_code tokens),
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from anan_cli.models import fetch_api_models
...
def _model_flow_minimax_oauth(config, current_model="", args=None):
    ...
    from anan_cli.models import _PROVIDER_MODELS
...
def _model_flow_custom(config):
    ...
    """
    from anan_cli.auth import _save_model_choice, deactivate_provider
    from anan_cli.config import get_env_value, load_config, save_config
    ...
    from anan_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `anan model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from anan_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from anan_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from anan_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  Anan will probe your")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from anan_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from anan_cli.config import save_env_value, load_config, save_config
    from anan_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from anan_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from anan_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Anan turns to `copilot --acp`.")
    print("  Anan currently starts its own ACP subprocess for each request.")
    print("  Anan uses your selected model as a hint for the Copilot ACP session.")
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``anan setup`` / ``anan model``.
    ...
    recover from a malformed paste without editing ``~/.anan/.env`` by hand.
    ...
    from anan_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `anan setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so anan knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Anan typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Anan, enable billing on your "
    ...
    if provider_id == "lmstudio":
        ...
        from anan_cli.models import fetch_lmstudio_models
        ...
        from anan_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from anan_constants import display_anan_home as _dhh_fn

            print(
                f"    Anan will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from anan_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'anan model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Anan CLI with a provider."""
    from anan_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from anan_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from anan_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from anan_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from anan_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from anan_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``anan slack <subcommand>``. Currently supports:
    ...
    if sub in (None, ""):
        ...
        print(
            "usage: anan slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `anan slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from anan_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from anan_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from anan_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from anan_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from anan_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from anan_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from anan_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Anan home directory to a zip file."""
    if getattr(args, "quick", False):
        from anan_cli.backup import run_quick_backup
...
def cmd_import(args):
    """Restore a Anan backup from a zip file."""
...
def cmd_version(args):
    """Show version."""
    print(f"Anan Agent v{__version__} ({__release_date__})")
    ...
    try:
        from anan_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall Anan Agent."""
    _require_tty("uninstall")
    from anan_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``anan update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``anan_cli/web_dist/`` (per vite.config.ts
    outDir: "../anan_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "anan_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``anan update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``anan web``).
    ...
    if r1.returncode != 0:
        print(
            ...
            + ("" if fatal else " (anan web will not be available)")
        )
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``anan dashboard`` processes other than ourselves.

    ``anan dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``anan update`` replaces files on disk, the running
    ...
    patterns = [
        "anan dashboard",
        "anan_cli.main dashboard",
        "anan_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "anan.*dashboard"`) keeps us consistent
            # with `anan_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `anan update`.
    ...
    print("  Preview now:  anan curator run --dry-run")
    print("  Pause it:     anan curator pause")
    print(
        "  Docs:         https://anan.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``anan dashboard`` processes.

    Called at the end of ``anan update`` (default ``reason``) and also
    from ``anan dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    anan dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Anan Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/anan/anan/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="anan-update-")
        zip_path = os.path.join(tmp_dir, f"anan-{branch}.zip")
        ...
        # GitHub ZIPs extract to anan-<branch>/
        extracted = os.path.join(tmp_dir, f"anan-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "anan-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Anan behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes anan completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Anan couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Anan couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Anan behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `anan update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/anan/anan.git",
    "git@github.com:anan/anan.git",
    "https://github.com/anan/anan",
    "git@github.com:anan/anan",
}
OFFICIAL_REPO_URL = "https://github.com/anan/anan.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_anan_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_anan_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Anan repository.")
        print("  This means you may miss updates from anan/anan.")
        print()
        try:
            response = (
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/anan/anan.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/anan/anan.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``anan update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from anan_constants import get_default_anan_root

    default_home = get_default_anan_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "anan[matrix]" or "package-name[extra]".
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``anan update`` is still progressing even if pip/uv itself is silent.
    """
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    nothing on fast hardware and prevents the "anan update hangs" reports
    on slow hardware.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``anan update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.anan/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``anan update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``anan update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.anan/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``anan update --gateway``) the update is already
    ...
    try:
        ...
        # anan_cli.config.get_anan_home to simulate setup failure.
        from anan_cli.config import get_anan_home as _get_anan_home

        logs_dir = _get_anan_home() / "logs"
...
def _cmd_update_check():
    """Implement ``anan update --check``: fetch and report without installing."""
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``anan update`` without requiring a reinstall.  The
    ...
    ``anan`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/anan.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v anan'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/anan, code at /usr/local/lib/anan).
    fhs_link = Path("/usr/local/bin/anan")
    ...
    path_comment = (
        "# Anan Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ANAN_HOME before running the update.
    ...
    ANAN_HOME directories.  The ``--backup`` flag on ``anan update``
    ...
    # Render path using display_anan_home so the user sees ~/.anan/...
    try:
        from anan_constants import get_anan_home, display_anan_home

        home = get_anan_home()
        try:
            display_path = f"{display_anan_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  anan import {out_path}")
...
def cmd_update(args):
    """Update Anan Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from anan_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Anan Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # though `git pull` can't touch $ANAN_HOME, this is cheap
        ...
        # the old bytecode (e.g. get_anan_home added to anan_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload anan_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_anan_home() added since the last release.
        try:
            import importlib
            import anan_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ANAN_HOME so
        # it is not affected by sync_skills()'s module-level ANAN_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ANAN_HOME env var points at the default or a named profile.
        try:
            from anan_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in ("", "y", "yes", "auto"):
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: anan config migrate")
            else:
                print()
                print("Skipped. Run 'anan config migrate' later to configure.")
        else:
        ...
        # When running as ``anan update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_anan_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all anan-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. anan-gateway.service or anan-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: anan gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: anan -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Anan gateway unit files are still installed.
        # When both anan.service (from a pre-rename install) and the
        # current anan-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `anan update` surfaces the issue until the user migrates.
        try:
            from anan_cli.gateway import (
                has_legacy_anan_units,
                _find_legacy_anan_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_anan_units():
                print()
                print("⚠ Legacy Anan gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_anan_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (anan.service) fight the current")
                print("  anan-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    anan gateway migrate-legacy")
                print()
        ...
        print("  anan model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``anan -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `anan profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_anan_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → anan -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.anan)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  anan profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     anan -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `anan profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from anan_cli.profiles import profile_exists
        ...
        from anan_cli.profiles import rename_profile
        ...
        from anan_cli.profiles import export_profile
        ...
        from anan_cli.profiles import import_profile
        ...
        import tempfile
        from anan_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="anan_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  anan -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      anan -p {plan.manifest.name} chat")
        ...
        from anan_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `anan profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"anan -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from anan_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("anan_requires"):
            print(f"Requires:     Anan {data['anan_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from anan_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.anan_requires:
        print(f"  Requires: Anan {mf.anan_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``anan dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``anan dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No anan dashboard processes running.")
        return 0

    print(f"{len(pids)} anan dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No anan dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `anan update`.
    ...
    from anan_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from anan_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Anan log files."""
    from anan_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from anan_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``anan -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``anan_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``anan -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``anan`` or only flags → defaults to ``chat``.
        return False
...
def main():
    ...
    from anan_cli._parser import build_top_level_parser
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: anan-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from anan_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://anan.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `anan model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy anan.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Anan gateway unit files "
            "(e.g. anan.service) left over from older installs. Profile "
            "units (anan-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Anan Agent with an interactive wizard. "
        "Run a specific section: anan setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'anan setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Anan.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $ANAN_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Anan")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Anan CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: anan-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Anan with Spotify via PKCE"
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Anan Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.anan/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.anan/scripts/. Pass empty string to clear. "
    ...
    # =========================================================================
    from anan_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.anan/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.anan/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Anan Agent setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Anan setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Anan Agent. Use 'anan debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    anan debug share              Upload debug report and print URL
    anan debug share --lines 500  Include more log lines
    anan debug share --expire 30  Keep paste for 30 days
    anan debug share --local      Print report locally (no upload)
    anan debug share --no-redact  Disable upload-time secret redaction
    anan debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'anan debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Anan home directory to a zip file",
        description="Create a zip archive of your entire Anan configuration, "
        "skills, sessions, and data (excludes the anan codebase). "
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.anan/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo anan uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from anan_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Anan backup from a zip file",
        description="Extract a previously created Anan backup into your "
        "Anan home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Anan Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from anan_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.anan/skills/.bundled_manifest) "
            "so future 'anan update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from anan_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from anan_cli.skills_hub import skills_command
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `anan plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from anan_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``anan --help``, ``anan version``, ``anan logs``,
    ...
    try:
        from anan_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_anan_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_anan_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_anan_home()}/memories/\n")
        else:
            from anan_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'anan tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # anan tools disable <name...> [--platform cli]
    ...
    # anan tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from anan_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from anan_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Anan as an MCP server",
        description=(
            "Manage MCP server connections and run Anan as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'anan mcp add' to connect to a new server, or\n"
            "'anan mcp serve' to expose Anan conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Anan as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `anan mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from anan_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch anan --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            from anan_cli.relaunch import relaunch
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Anan",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Anan",
        description="Import settings, memories, skills, and API keys from an OpenClaw installation. "
        "Always shows a preview before making changes.",
    )
    claw_migrate.add_argument(
        "--source", help="Path to OpenClaw directory (default: ~/.anan)"
    )
    claw_migrate.add_argument(
        "--dry-run",
        action="store_true",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.anan/ (by default a "
        "single restore-point archive is written to ~/.anan/backups/ "
        "before apply; restorable with 'anan import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from anan_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Anan Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'anan config migrate' separately for those.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Anan Agent",
        description="Remove Anan Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Anan Agent as an ACP (Agent Client Protocol) server",
        description="Start Anan Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch Anan Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Anan instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `anan update` skill sync)",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Anan profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Anan Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `anan --tui` via PTY/WebSocket). "
    ...
    # for `anan dashboard` cmdlines and SIGTERM them directly — the same
    # path `anan update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running anan dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running anan dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Anan log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    anan logs                    Show last 50 lines of agent.log
    anan logs -f                 Follow agent.log in real time
    anan logs errors             Show last 50 lines of errors.log
    anan logs gateway -n 100     Show last 100 lines of gateway.log
    anan logs --level WARNING    Only show WARNING and above
    anan logs --session abc123   Filter by session ID
    anan logs --component tools  Only show tool-related lines
    anan logs --since 1h         Lines from the last hour
    anan logs --since 30m -f     Follow, starting from 30 min ago
    anan logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``anan -c Pokemon Agent Dev`` → ``anan -c 'Pokemon Agent Dev'``
    ...
    from anan_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'anan -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (anan hooks list, cron
    ...
    if getattr(args, "oneshot", None):
        from anan_cli.oneshot import run_oneshot
