...
Subcommands:
  anan fallback [list]   Show the current fallback chain (default when no subcommand)
  anan fallback add      Pick provider + model via the same picker as `anan model`,
                           then append the selection to the chain
  anan fallback remove   Pick an entry to delete from the chain
  anan fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.anan/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from anan_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `anan model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  anan fallback add")
        print()
        return
    ...
    print("  Docs: https://anan.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `anan model`, then append the selection to the chain."""
    from anan_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `anan model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `anan fallback list` to view, or `anan fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``anan fallback [subcommand]``."""
