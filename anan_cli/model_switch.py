...
- ``anan_cli.providers``        -- canonical provider identity + overlays
- ``anan_cli.model_normalize``  -- per-provider name formatting
...
_SINOCLAW_MODEL_WARNING = (
    "Nous Research Anan 3 & 4 models are NOT agentic and are not designed "
    "for use with Anan Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Anan 3 / Anan 4 chat families.
...
#   NousResearch/Anan-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
...
def is_nous_anan_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Anan 3/4 chat model.
...
def _check_anan_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Anan 3/4 chat model."""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``anan config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from anan_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'anan model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'anan doctor' — config issues detected:"
    ...
    # Mirrors the same logic in anan_cli.runtime_provider.resolve_runtime_provider;
    ...
    anan_warn = _check_anan_model_warning(new_model)
    if anan_warn:
        warnings.append(anan_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from anan_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from anan_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by anan provider ID
    ...
    # --- 1. Check Anan-mapped providers ---
    for anan_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(anan_id)
        ...
        if not has_creds:
            try:
                from anan_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and anan_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a Anan release.
        model_ids = curated.get(anan_id, [])
        if anan_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(anan_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Anan-only providers (nous, openai-codex, copilot, opencode-go) ---
    from anan_cli.providers import SINOCLAW_OVERLAYS
    from anan_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Anan provider ID.
    # SINOCLAW_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Anan IDs ("copilot").
    ...
    for pid, overlay in SINOCLAW_OVERLAYS.items():
        ...
        # Resolve Anan slug — e.g. "github-copilot" → "copilot"
        anan_slug = _mdev_to_hermes.get(pid, pid)
        if anan_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(anan_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, anan_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or anan_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(anan_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", anan_slug, exc)
        ...
        # configured.
        if not has_creds and anan_slug == "anthropic":
            try:
                ...
                if (anan_creds and anan_creds.get("accessToken")) or \
        ...
        if anan_slug in {"copilot", "copilot-acp"}:
            model_ids = provider_model_ids(anan_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(anan_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(anan_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by Anan slug, fall back to overlay key
            model_ids = curated.get(anan_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if anan_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(anan_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": anan_slug,
            "name": get_label(anan_slug),
            "is_current": anan_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "hermes",
        ...
        seen_slugs.add(anan_slug.lower())
        _record_builtin_endpoint(anan_slug)
    ...
    # with `anan model`).
    try:
        from anan_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Anan's canonical write key (matches
            ...
            # Anan writes ``models:`` as a dict keyed by model id
            # (see anan_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Anan's own writer uses; a hyphen variant is accepted
            ...
            # active model. Anan's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, anan_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`anan_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from anan_cli.models import fetch_openrouter_models
