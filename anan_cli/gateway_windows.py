...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m anan_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Anan_Gateway"
_TASK_DESCRIPTION = "Anan Agent Gateway - Messaging Platform Integration"
...
def get_task_name() -> str:
    ...
    Default profile: ``Anan_Gateway``
    Named profile X: ``Anan_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during anan_cli boot.
    from anan_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    (or ``<ANAN_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Anan installs stay self-contained).
    """
    _assert_windows()
    from anan_cli.config import get_anan_home

    script_dir = Path(get_anan_home()) / "gateway-service"
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports ANAN_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``python -m anan_cli.main [--profile X] gateway run --replace``
    ...
    lines.append(f'set "ANAN_HOME={anan_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports anan_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    prog_args = [python_path, "-m", "anan_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    anan_home = str(Path(get_anan_home()).resolve())
    profile_arg = _profile_arg(anan_home)

    content = _build_gateway_cmd_script(python_path, working_dir, anan_home, profile_arg)
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "anan_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m anan_cli.main gateway run --replace``
    ...
    log_dir = Path(get_anan_home()) / "logs"
...
def _wait_for_gateway_ready(timeout_s: float = 6.0, interval_s: float = 0.4) -> list[int]:
    ...
    """
    from anan_cli.gateway import find_gateway_pids
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from anan_cli.config import get_anan_home
        print(f"    type {Path(get_anan_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_anan_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from anan_cli.config import get_anan_home

    anan_home = Path(get_anan_home()).resolve()
    print()
    print("Next steps:")
    print("  anan gateway status                      # Check status")
    print(f"  type {anan_home}\\logs\\gateway.log       # View logs")
...
def stop() -> None:
    ...
    _assert_windows()
    from anan_cli.gateway import kill_gateway_processes
