...
``<root>`` is the **shared Anan root** (the parent of any active
...
``anan -p <profile>`` joins the same board as the dispatcher that
...
the profile's Anan home but are otherwise isolated: a worker spawned
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``anan kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.anan``. In Docker / custom
deployments where ``ANAN_HOME`` points outside ``~/.anan`` (e.g.
``/opt/hermes``), ``<root>`` is ``ANAN_HOME``. Legacy env-var
...
# enough that kebab-case names like ``atm10-server`` or ``anan``
...
def kanban_home() -> Path:
    """Return the shared Anan root that anchors the kanban board.

    Resolution order:

    1. ``SINOCLAW_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_anan_root()``, which already returns ``<root>``
       when ``ANAN_HOME`` is ``<root>/profiles/<name>``, and returns
       ``ANAN_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``ANAN_HOME`` would silently fork the board per profile,
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``anan kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``anan kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``anan kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``anan kanban log`` unambiguous even when multiple
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    # See anan_state._WAL_INCOMPAT_MARKERS for detection logic.
    from anan_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``anan kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from anan_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``anan --skills ...`` alongside the built-in
    ...
    # `anan --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``anan kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``anan kanban complete <ready-task>
...
def complete_task(
    ...
    *,
    ...
    completion (``anan kanban complete <id>``) works without requiring
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Anan
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``anan kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Anan profile.
    ...
    if not rows:
        return False
    try:
        from anan_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Anan profile.
        # `_default_spawn` invokes ``anan -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Anan
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``anan -p <profile> chat -q ...`` subprocess.
    ...
    # that even when the worker activates a profile (`anan -p <name>`
    # rewrites ANAN_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_anan_root()`
    ...
    # `anan -p <assignee>` activates the profile, but the env var is
    ...
    # `anan kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install Anan Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``anan kanban daemon`` is systemd-friendly.
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Anan root exists

    Reads profile paths directly so this module has no import dependency on
    ``anan_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from anan_constants import get_default_anan_root
        default_root = get_default_anan_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``anan kanban assignees`` for the terminal.
