...
on Japanese locales, etc.).  Anan's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``anan_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Anan is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Anan-managed Git install directories so
    ...
    # install without Anan-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Anan's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Anan's
    ``anan config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Anan (User env var in Windows Settings, or
    export in a PowerShell profile) and Anan picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Anan-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Any subprocess Anan spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Anan-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # Anan venv Scripts directory — host of the hermes.exe shim itself,
        ...
        os.path.join(local_appdata, "hermes", "anan", "venv", "Scripts"),
