"""`anan checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.anan/checkpoints/``.  Actions:

    anan checkpoints               # same as `status`
    anan checkpoints status        # total size, project count, breakdown
    anan checkpoints list          # per-project checkpoint counts + workdir
    anan checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    anan checkpoints clear [-f]    # nuke the entire base (asks first)
    anan checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    anan checkpoints
    anan checkpoints prune --retention-days 3 --max-size-mb 200
    anan checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: anan checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``anan checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `anan checkpoints` → status
