"""
Single source of truth for provider identity in Anan Agent.
...
2. **Anan overlays** — transport type, auth patterns, aggregator flags,
...
# -- Anan overlay ----------------------------------------------------------
# Anan-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class AnanOverlay:
    """Anan-specific provider metadata layered on top of models.dev."""
...
SINOCLAW_OVERLAYS: Dict[str, AnanOverlay] = {
    "openrouter": AnanOverlay(
    ...
    "openai-codex": AnanOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": AnanOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="SINOCLAW_QWEN_BASE_URL",
    ),
    "google-gemini-cli": AnanOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": AnanOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": AnanOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": AnanOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": AnanOverlay(
    ...
    "minimax-oauth": AnanOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": AnanOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": AnanOverlay(
        transport="openai_chat",
    ...
    "alibaba-coding-plan": AnanOverlay(
        transport="openai_chat",
    ...
    "opencode": AnanOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": AnanOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Anan overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Anan overlay
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + anan extra
    ...
    if overlay is not None:
        # Anan-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
