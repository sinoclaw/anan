"""
Configuration management for Anan Agent.

Config files are stored in ~/.anan/ for easy access:
- ~/.anan/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.anan/.env         - API keys and secrets

This module provides:
- anan config          - Show current configuration
- anan config edit     - Open config in editor
- anan config set      - Set a specific value
- anan config wizard   - Re-run setup wizard
"""
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `anan plugins enable
    # observability/langfuse` or `anan tools → Langfuse`); credentials gate
...
from anan_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_anan_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Anan is running in package-manager-managed mode.

    Two signals: the SINOCLAW_MANAGED env var (set by the systemd service),
    or a .managed marker file in ANAN_HOME (set by the NixOS activation
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "anan update"


def format_managed_message(action: str = "modify this Anan installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this Anan installation is managed by NixOS "
            f"(SINOCLAW_MANAGED={env_hint}).\n"
            "Edit services.anan.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Anan installation is managed by Homebrew "
            f"(SINOCLAW_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade anan"
        )
    ...
    return (
        f"Cannot {action}: this Anan installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Anan."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ANAN_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, anan_bin
    ...
    container_mode_file = get_anan_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "anan")
    exec_user = info.get("exec_user", "hermes")
    anan_bin = info.get("anan_bin", "/data/current-package/bin/anan")
...
# Re-export from anan_constants — canonical definition lives there.
from anan_constants import get_anan_home  # noqa: F811,E402
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the anan group can
    share state with the gateway service.

    The mode can be overridden via the ANAN_HOME_MODE environment variable
    (e.g. ANAN_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ANAN_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ANAN_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Anan runs in a container with volume-mounted config files, forcing
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ANAN_HOME if the user doesn't have one yet."""
...
def ensure_anan_home():
    """Ensure ~/.anan directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_anan_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_anan_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ANAN_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Anan-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Anan auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Anan sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Anan
        ...
        # bundled Anan image, which drops to the `anan` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Anan sends a stable profile-scoped userId to Camofox
    ...
    # Opt in via ``anan chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.anan/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: anan sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Anan truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``anan kanban specify`` (single id or --all). Set a
        ...
        # model; override via `anan model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `anan --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `anan -c` muscle memory.  Default off so existing
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.anan/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.anan/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Anan feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Anan auto-pauses the goal and
    ...
    # cron daemon.
    #
    # See `anan curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.anan/skills/ into
        # ~/.anan/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `anan curator rollback`.
        "backup": {
            "enabled": True,
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Anan Agent*" header.
    ...
    # stored approval from ~/.anan/shell-hooks-allowlist.json.
    ...
    # todos to ready, and fires `anan -p <assignee> chat -q ...` for
    ...
    "code_execution": {
        ...
        #     anan's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.anan/logs/.
    ...
    # update model picker lists without shipping a anan release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://anan.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `anan model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.anan/state.db.
    ...
    "sessions": {
        ...
        # default of ``anan sessions prune``.
        "retention_days": 90,
    ...
    # ``anan update`` behaviour.
    "updates": {
        # Run a full ``anan backup``-style zip of ANAN_HOME before every
        # ``anan update``.  Backups land in ``<ANAN_HOME>/backups/`` and
        # can be restored with ``anan import <path>``.  Off by default —
        # on large ANAN_HOME directories the zip can add minutes to every
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'anan model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Anan auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @anan:example.org)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #anan)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: anan-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'anan' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Anan API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Anan API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Anan API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.anan.config`` entries, then checks
    ...
    try:
        ...
        # should never break `anan update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Anan versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``anan_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'anan setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Anan won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'anan doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ANAN_HOME", "~/.anan")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ANAN_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_anan_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `anan plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `anan curator status` has no
    ...
    #   3. Creates `~/.anan/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_anan_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_anan_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: anan config set <key> <value>")
    ...
    # metadata.anan.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _anan_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.anan/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    ...
    (which change ``ANAN_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    with _CONFIG_LOCK:
        ensure_anan_home()
...
#   openai-codex (OAuth — anan auth) — OpenAI Codex
#   nous         (OAuth — anan auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.anan/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.anan/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Anan env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.anan/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.anan/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.anan/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.anan/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Anan — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Anan vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.anan/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Anan Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  anan config edit     # Edit config file", Colors.DIM))
    print(color("  anan config set <key> <value>", Colors.DIM))
    print(color("  anan setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: anan config set <key> <value>")
            print()
            print("Examples:")
            print("  anan config set model anthropic/claude-sonnet-4")
            print("  anan config set terminal.backend docker")
            print("  anan config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'anan config migrate' to add them")
        ...
        print("  anan config           Show current configuration")
        print("  anan config edit      Open config in editor")
        print("  anan config set <key> <value>   Set a config value")
        print("  anan config check     Check for missing/outdated config")
        print("  anan config migrate   Update config with new options")
        print("  anan config path      Show config file path")
        print("  anan config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``anan config`` UI so users
