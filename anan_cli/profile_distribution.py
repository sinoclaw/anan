"""Profile distributions — shareable, packaged Anan profiles via git.

A distribution is a Anan profile published as a git repository (or
...
* ``anan profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``anan skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``anan profile``, not a parallel tree):

    anan profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "anan", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            anan_requires=str(data.get("anan_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.anan_requires:
            out["anan_requires"] = self.anan_requires
        if self.author:
...
def check_anan_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Anan {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Anan distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``anan profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Anan profile distribution."
            )
...
def plan_install(
    ...
    from anan_cli import __version__ as anan_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Anan distribution."
        )

    # Version check up-front so we fail fast
    check_anan_requires(manifest.anan_requires, anan_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.anan).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="anan_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `anan profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`anan profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="anan_dist_update_") as tmp:
        plan = plan_install(
