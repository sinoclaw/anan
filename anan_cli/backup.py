"""
Backup and import commands for anan CLI.

`anan backup` creates a zip archive of the entire ~/.anan/ directory
(excluding the anan repo and transient files).

`anan import` restores from a backup zip, overlaying onto the current
ANAN_HOME root.
"""
...
from anan_constants import get_default_anan_root, get_anan_home, display_anan_home
...
_EXCLUDED_DIRS = {
    "anan",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to anan root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the Anan home directory."""
    anan_root = get_default_anan_root()

    if not anan_root.is_dir():
        print(f"Error: Anan home directory not found at {anan_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"anan-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"anan-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_anan_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(anan_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(anan_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(anan_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside anan root
            try:
    ...
    print(f"\nRestore with: anan import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Anan backup.
    ...
    # Look for telltale files that a anan home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Anan backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.anan/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a anan dir name
        if prefix in (".anan", "anan"):
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_anan_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (anan_root / "config.yaml").exists()
        has_env = (anan_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Anan configuration.")
        ...
        anan_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            target = anan_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(anan_root.resolve())
            except ValueError:
        ...
        profiles_dir = anan_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # anan_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: anan profile list  (after installing hermes)")

        # Guidance
        print()
        if not (anan_root / "anan").is_dir():
            print("Note: The anan codebase was not included in the backup.")
            print("  If this is a fresh install, run: anan update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  anan -p {pname} gateway install")

        print("Done. Your Anan configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and anan backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ANAN_HOME).
...
# — `anan update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(anan_home: Optional[Path] = None) -> Path:
    home = anan_home or get_anan_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    anan_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(anan_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(anan_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for anan backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_anan_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, anan_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``anan_root`` to ``out_path``.
...
def _pre_update_backup_dir(anan_home: Optional[Path] = None) -> Path:
    home = anan_home or get_anan_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ANAN_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ANAN_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``anan update``) should continue even if the backup fails.
    """
    anan_root = anan_home or get_default_anan_root()
    if not anan_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(anan_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, anan_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `anan claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ANAN_HOME under ``backups/`` before a
    ``anan claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``anan import <archive>``.  Writes to
    ``<ANAN_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `anan import` and the
