"""
Status command for anan CLI.

Shows the status of all Anan Agent components.
"""
...
from anan_cli.auth import AuthError, resolve_provider
from anan_cli.colors import Colors, color
from anan_cli.config import get_env_path, get_env_value, get_anan_home, load_config
from anan_cli.models import provider_label
from anan_cli.nous_subscription import get_nous_subscription_features
from anan_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``anan config``'s
...
def show_status(args):
    """Show status of all Anan Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Anan Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: anan auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: anan model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: anan auth add minimax-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: anan model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from anan_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'anan[vercel]')"
    ...
    try:
        from anan_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_anan_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_anan_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'anan doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'anan setup' to configure", Colors.DIM))
    print()
