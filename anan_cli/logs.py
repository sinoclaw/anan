"""``anan logs`` — view and filter Anan log files.
...
Usage examples::
    ...
    anan logs                    # last 50 lines of agent.log
    anan logs -f                 # follow agent.log in real time
    anan logs errors             # last 50 lines of errors.log
    anan logs gateway -n 100    # last 100 lines of gateway.log
    anan logs --level WARNING    # only WARNING+ lines
    anan logs --session abc123   # filter by session ID substring
    anan logs --component tools  # only tool-related lines
    anan logs --since 1h         # lines from the last hour
    anan logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_anan_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Anan runs — try 'hermes chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_anan_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_anan_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_anan_home()}/logs/")
        return

    print(f"Log files in {display_anan_home()}/logs/:\n")
    found = False
