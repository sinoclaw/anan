...
Usage:
    anan pairing list              # Show all pending + approved users
    anan pairing approve <platform> <code>  # Approve a pairing code
    anan pairing revoke <platform> <user_id> # Revoke user access
    anan pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle anan pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: anan pairing {list|approve|revoke|clear-pending}")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.anan/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
