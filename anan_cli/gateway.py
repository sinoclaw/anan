"""
Gateway subcommand for anan CLI.

Handles: anan gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_anan_home is imported lazily at call sites to avoid ImportError
# when anan_constants is cached from a pre-update version during `anan update`.
from anan_cli.setup import (
    print_header, print_info, print_success, print_warning, print_error,
    prompt, prompt_choice, prompt_yes_no,
)
from anan_cli.colors import Colors, color
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "anan-gateway*",
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``anan gateway status`` from falsely counting the ``hermes`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``anan gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "anan_cli.main gateway",
        "anan_cli.main --profile",
        "anan_cli.main -p",
        "anan_cli/main.py gateway",
        "anan_cli/main.py --profile",
        "anan_cli/main.py -p",
        "anan gateway",
        "gateway/run.py",
    ]
    current_home = str(get_anan_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"ANAN_HOME={current_home}" in command
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. ANAN_HOME
        ...
        # ANAN_HOME= in argv is.
        if "--profile " in command or " -p " in command:
            return False
        if "ANAN_HOME=" in command and f"ANAN_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``anan update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Anan profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Anan profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "anan_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``anan update`` users
    # with no running gateway until they re-invoke ``anan gateway``
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}anan gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}anan gateway restart{scope_flag}")
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from anan_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `anan gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``anan gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from anan_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ANAN_HOME-scoped).
...
from anan_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "anan-gateway"
SERVICE_DESCRIPTION = "Anan Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ANAN_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ANAN_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from anan_constants import get_default_anan_root
    home = get_anan_home().resolve()
    default = get_default_anan_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ANAN_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(anan_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when ANAN_HOME is a named profile.

    For ``~/.anan/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        anan_home: Optional explicit ANAN_HOME path. Defaults to the current
            ``get_anan_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from anan_constants import get_default_anan_root
    home = Path(anan_home or str(get_anan_home())).resolve()
    default = get_default_anan_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this ANAN_HOME.

    Default ``~/.anan`` returns ``anan-gateway`` (backward compatible).
    Profile ``~/.anan/profiles/coder`` returns ``anan-gateway-coder``.
    Any other ANAN_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Anan installs that predate the
# anan-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (anan-gateway-*.service) and unrelated third-party
# "hermes" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("anan.service",)
...
def _find_legacy_anan_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Anan gateway units.

    Detects unit files installed by older Anan versions that used a
    different service name (e.g. ``anan.service`` before the rename to
    ``anan-gateway.service``). When both a legacy unit and the current
    ``anan-gateway.service`` are active, they fight over the same bot
...
def has_legacy_anan_units() -> bool:
    """Return True when any legacy Anan gateway unit files exist."""
    return bool(_find_legacy_anan_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Anan gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_anan_units()
    if not legacy:
        return
    print_warning("Legacy Anan gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current anan-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    anan gateway migrate-legacy")


def remove_legacy_anan_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Anan gateway unit files.

    Iterates over whatever ``_find_legacy_anan_units()`` returns — which is
    ...
    legacy = _find_legacy_anan_units()
    if not legacy:
        print("No legacy Anan gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Anan gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: anan gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo anan gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    anan gateway uninstall")
    print_info("    sudo anan gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so Anan can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo anan gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo anan gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo anan gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Anan often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.anan`` → ``ai.anan.gateway.plist`` (backward compatible).
    Profile ``~/.anan/profiles/coder`` → ``ai.anan.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.anan.gateway-{suffix}" if suffix else "ai.anan.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.anan/anan  -> /home/alice/.anan/anan
...
def _anan_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ANAN_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_anan_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.anan                    → /home/alice/.anan
      /root/.anan/profiles/coder     → /home/alice/.anan/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    current_hermes = get_anan_home().resolve()
    current_default = (Path.home() / ".anan").resolve()
    target_default = Path(target_home_dir) / ".anan"

    # Default ~/.anan → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.anan → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.anan) — keep as-is
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        anan_home = _anan_home_for_target_user(home_dir)
        profile_arg = _profile_arg(anan_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m anan_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="ANAN_HOME={anan_home}"
Restart=always
RestartSec=60
...
"""
    ...
    anan_home = str(get_anan_home().resolve())
    profile_arg = _profile_arg(anan_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Anan install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo anan gateway uninstall --system")
    print_info("         anan gateway install")
    print_info("         anan gateway start")
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # Offer to remove legacy units (anan.service from pre-rename installs)
    # before installing the new anan-gateway.service. If both remain, they
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}anan gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}anan gateway status{scope_flag}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}anan gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `anan gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `anan gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}anan gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}anan gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}anan gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}anan gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.anan.gateway-{suffix}" if suffix else "ai.anan.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Anan install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  anan gateway status             # Check status")
    from anan_constants import display_anan_home as _dhh
...
def launchd_stop():
    ...
    # `anan gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Anan install")
    else:
        print("⚠ Service definition is stale relative to the current Anan install")
        print("  Run: anan gateway start")
    ...
    if deep:
        log_file = get_anan_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the Anan gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'hermes' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the Anan command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$ANAN_HOME and break later non-root dashboard/gateway runs."
    )
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # process (via ``anan gateway install`` → Scheduled Task / Startup
    ...
    # Wrong. When the user runs `anan gateway start` from a PowerShell
    ...
    # `hermes` invocation (like `anan gateway status` 30 seconds later)
    # because Windows routes console events to all processes sharing the
    # console. Every anan CLI process after that sibling fires is a
    ...
    # /restart) rather than through `anan gateway restart` which already
    ...
    # `anan gateway start/restart` — leaving the gateway vulnerable to
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("SINOCLAW_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from anan_constants import get_anan_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Anan will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @anan:matrix.example.org"},
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Anan delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Anan agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Anan will use (e.g., hermes@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Anan connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: anan pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Anan will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``anan setup gateway`` without needing the gateway to be running.
    ...
    # User-installed platform plugins under ~/.anan/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_anan_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: anan pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'anan gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from anan_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from anan_cli.dingtalk_auth import dingtalk_qr_auth
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from anan_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Anan will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Anan will store the returned account_id/token in ~/.anan/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `anan gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_anan_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `anan pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `anan pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"AnanAgent\"")
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``anan_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from anan_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.anan/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.anan/.env: {', '.join(required)}")
    else:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: anan gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    try:
                        ...
                        print_info("  You can try manually: anan gateway install")
                else:
                    print_info("  You can install later: anan gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo anan gateway install --system")
                    print_info("  Or run in foreground:  anan gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: anan gateway run")
                print_info("  For persistence:   tmux new -s anan 'anan gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: anan gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup anan gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: anan gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'anan gateway setup' when ready.")
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``anan gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: anan gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: anan gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s anan 'anan gateway run'")
                print()
            ...
            print()
            print("  anan gateway run                              # direct foreground")
            print("  tmux new -s anan 'anan gateway run'         # persistent via tmux")
            print("  nohup anan gateway run > ~/.anan/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: anan gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: anan gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: anan gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: anan gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: anan gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  anan gateway install")
                else:
                    print("To install as a service:")
                    print("  anan gateway install")
                    print("  sudo anan gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  anan gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup anan gateway run > ~/.anan/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s anan 'anan gateway run'         # persistent via tmux")
                    print("  nohup anan gateway run > ~/.anan/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  anan gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  anan gateway install  # Install as user service")
                    print("  sudo anan gateway install --system  # Install as boot-time system service")
        ...
        # Stop, disable, and remove legacy Anan gateway unit files from
        # pre-rename installs (e.g. anan.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_anan_units(interactive=not yes, dry_run=dry_run)
