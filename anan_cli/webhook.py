...
Usage:
    ...
    anan webhook list
    anan webhook remove <name>
    anan webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.anan/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _anan_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: anan gateway run
"""
...
def webhook_command(args):
    ...
    if not sub:
        print("Usage: anan webhook {subscribe|list|remove|test}")
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (anan gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: anan webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from anan webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (anan gateway run)")
