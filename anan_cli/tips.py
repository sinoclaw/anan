...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or anan -c.",
    ...
    "Ctrl+Z suspends Anan to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "hermes -c resumes your most recent CLI session. anan -c \"project name\" resumes by title.",
    ...
    "anan chat -t web,terminal enables only specific toolsets for a focused session.",
    "anan chat -s github-pr-workflow preloads a skill at launch.",
    "anan chat -q \"query\" runs a single non-interactive query and exits.",
    "anan chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "anan chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hermes --yolo bypasses all dangerous command approval prompts for the entire session.",
    "anan chat --source telegram tags the session for filtering in anan sessions list.",
    "anan -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "anan doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "anan dump outputs a compact setup summary — great for bug reports.",
    "anan config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "anan config edit opens config.yaml in your default editor.",
    "anan config check scans for missing or stale configuration options.",
    "anan sessions browse opens an interactive session picker with search.",
    "anan sessions stats shows session counts by platform and database size.",
    "anan sessions prune --older-than 30 cleans up old sessions.",
    "anan skills search react --source skills-sh searches the skills.sh public directory.",
    "anan skills check scans installed hub skills for upstream updates.",
    "anan skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "anan skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "anan mcp add github --command npx adds MCP servers from the command line.",
    "anan mcp serve runs Anan itself as an MCP server for other agents.",
    "anan auth add lets you add multiple API keys for credential pool rotation.",
    "anan completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "anan logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "anan backup creates a zip backup of your entire Anan home directory.",
    "anan profile create coder creates an isolated profile that becomes its own command.",
    "anan profile create work --clone copies your current config and keys to a new profile.",
    "anan update syncs new bundled skills to ALL profiles automatically.",
    "anan gateway install sets up Anan as a system service (systemd/launchd).",
    "anan memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "anan webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: anan tools disables unused tools, anan skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "anan models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without changing main chat.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as anan -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Anan tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'anan profile create coder' creates the 'coder' command.",
    "anan profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Anan prints a resume command with session ID and stats.",
    "anan sessions export backup.jsonl exports all sessions for backup or analysis.",
    ...
    "anan skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .anan/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: anan cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Anan runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "anan gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Anan via a local macOS server.",
    ...
    "Context files (.anan.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.anan/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Anan loads project context from .anan.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — anan tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.anan/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "anan chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "anan plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "anan acp runs Anan as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "anan login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "anan status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.anan/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Anan your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.anan/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Anan auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.anan/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at ANAN_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "ANAN_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ANAN_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.anan/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'anan profile create ls' would be rejected.",
    "anan profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "anan claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via anan tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.anan/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.anan/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Anan auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Anan config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Anan is working.',
    ...
    '/reload reloads ~/.anan/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.anan/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.anan/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'anan curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "anan curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'anan curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'anan auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'anan dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'anan dashboard --tui embeds the full Anan TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.anan/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.anan/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.anan/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.anan/skills/ so drop-in skills appear without restarting the session.',
    ...
    'anan chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'anan chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'anan chat --ignore-user-config skips ~/.anan/config.yaml — reproducible bug reports and CI runs.',
    "anan chat --source tool tags programmatic chats so they don't clutter anan sessions list.",
    'anan dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'anan sessions rename <ID> "new title" renames any past session; anan sessions delete <ID> removes one.',
    'anan import restores a session export or profile archive produced by sessions export or profile export.',
    'anan fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'anan pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'anan setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'anan status --deep runs the full health sweep across every component; plain anan status is the quick view.',
    ...
    'SINOCLAW_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.anan/plugins/ — trust-gated by design.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Anan block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from anan sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.anan/dashboard-plugins/.',
]
