"""
Profile management for multiple isolated Anan instances.

Each profile is a fully independent ANAN_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.anan/profiles/<name>/`` by default.

The "default" profile is ``~/.anan`` itself — backward compatible,
zero migration needed.

Usage::

    anan profile create coder          # fresh profile + bundled skills
    anan profile create coder --clone  # also copy config, .env, SOUL.md, skills
    anan profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    anan -p coder chat                 # or via flag
    anan profile use coder             # set as sticky default
    anan profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See anan_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Marker file written by `anan profile create --no-skills`.  When present in
...
# `anan skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Anan infrastructure directories that only the default profile
         (``~/.anan``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_anan_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.anan) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "anan",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Anan subcommands that cannot be used as profile names/aliases
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the anan root, NOT to the current ANAN_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ANAN_HOME points outside
    ``~/.anan``, profiles live under ``ANAN_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_anan_home() / "profiles"


def _get_default_anan_home() -> Path:
    """Return the default (pre-profile) ANAN_HOME path.

    In standard deployments this is ``~/.anan``.
    In Docker/custom deployments where ANAN_HOME is outside ``~/.anan``
    (e.g. ``/opt/data``), returns ANAN_HOME directly.
    """
    from anan_constants import get_default_anan_root
    return get_default_anan_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_anan_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Anan installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ANAN_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_anan_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, anan subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _SINOCLAW_SUBCOMMANDS:
        return f"'{canon}' conflicts with a anan subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``anan profile list`` for an unrelated profile.
    """
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_anan_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``anan update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.anan)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.anan/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `anan update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`anan profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `anan update`.\n",
                encoding="utf-8",
            )
        except OSError:
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ANAN_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``anan profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ANAN_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.anan).\n"
            "To remove everything, use: anan uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ANAN_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ANAN_HOME")
    try:
        os.environ["ANAN_HOME"] = str(profile_dir)
        from anan_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ANAN_HOME"] = old_home
        elif "ANAN_HOME" in os.environ:
            del os.environ["ANAN_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.anan/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: anan profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ANAN_HOME.

    Returns ``"default"`` if ANAN_HOME is not set or points to ``~/.anan``.
    Returns the profile name if ANAN_HOME points into ``~/.anan/profiles/<name>``.
    Returns ``"custom"`` if ANAN_HOME is set to an unrecognized path.
    """
    from anan_constants import get_anan_home
    anan_home = get_anan_home()
    resolved = anan_home.resolve()

    default_resolved = _get_default_anan_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.anan itself — its parent is ~/ and its
        # directory name is ".anan", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: anan profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.anan itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.anan). "
            "Specify a different name: anan profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="anan_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        new_dir / "honcho.json",
        _get_default_anan_home() / "honcho.json",
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for anan profile names."""
    return '''# Anan Agent profile completion
# Add to ~/.bashrc: eval "$(hermes completion bash)"

_anan_profiles() {
    local profiles_dir="$HOME/.anan/profiles"
...
_anan_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_anan_profiles)" -- "$cur"))
        return
    fi
...
complete -F _anan_completion hermes
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for anan profile names."""
    return '''#compdef hermes
# Anan Agent profile completion
# Add to ~/.zshrc: eval "$(hermes completion zsh)"

_hermes() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.anan/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.anan/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ANAN_HOME path string.

    Called early in the CLI entry point, before any anan modules
    are imported, to set the ANAN_HOME environment variable.
    """
