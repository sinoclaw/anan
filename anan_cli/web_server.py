"""
Anan Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m anan_cli.main web          # Start on http://127.0.0.1:9119
    python -m anan_cli.main web --port 8080
"""
...
WEB_DIST = Path(os.environ["ANAN_WEB_DIST"]) if "ANAN_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Anan Agent", version=__version__)
...
_SESSION_HEADER_NAME = "X-Anan-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``anan dashboard --tui``
...
# streamed to a per-action log file under ``~/.anan/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_anan_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "anan-update": "anan-update.log",
}
...
def _spawn_anan_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``anan <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``anan_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "anan_cli.main", *subcommand]
...
async def restart_gateway():
    """Kick off a ``anan gateway restart`` in the background."""
    try:
        proc = _spawn_anan_action(["gateway", "restart"], "gateway-restart")
...
async def update_hermes():
    """Kick off ``anan update`` in the background."""
    try:
        proc = _spawn_anan_action(["update"], "anan-update")
    except Exception as exc:
        _log.exception("Failed to spawn anan update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Anan supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in anan_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.anan/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``anan auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Anan resolves Anthropic creds in this order at runtime:
    1. ``~/.anan/.anthropic_oauth.json`` — Anan-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_anan_oauth_credentials,
            read_claude_code_credentials,
            _SINOCLAW_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_anan_oauth_credentials = None  # type: ignore
        _SINOCLAW_OAUTH_FILE = None  # type: ignore

    anan_creds = None
    if read_anan_oauth_credentials:
        try:
            anan_creds = read_anan_oauth_credentials()
        except Exception:
            anan_creds = None
    if anan_creds and anan_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "anan_pkce",
            "source_label": f"Anan PKCE ({_SINOCLAW_OAUTH_FILE})",
            "token_preview": _truncate_token(anan_creds.get("accessToken")),
            "expires_at": anan_creds.get("expiresAt"),
            "has_refresh_token": bool(anan_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Anan even
    when they also have a separate Anan-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "anan auth add nous",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from anan_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("anan_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Anan-managed PKCE file
    ...
    if provider_id in ("anthropic", "claude-code"):
        ...
        try:
            from anan_cli.auth import clear_provider_auth
...
#          → persists to ~/.anan/.anthropic_oauth.json AND credential pool
...
# Guarded so anan web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Anan file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``anan auth add anthropic``.
    """
    from agent.anthropic_adapter import _SINOCLAW_OAUTH_FILE
    payload = {
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    """
    if provider_id == "nous":
        from anan_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    from anan_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        from anan_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from anan_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_anan_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from anan_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_anan_home()
...
def _resolve_profile_dir(name: str) -> Path:
    ...
    from anan_cli import profiles as profiles_mod
    try:
...
def _profile_setup_command(name: str) -> str:
    ...
    return "anan setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from anan_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from anan_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
#
# The endpoint spawns the same ``anan --tui`` binary the CLI uses, behind
...
try:
    from anan_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``anan --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``SINOCLAW_TUI_RESUME`` env var —
    matching what ``anan_cli.main._launch_tui`` does for the CLI path.
    ...
    """
    from anan_cli.main import PROJECT_ROOT, _make_tui_argv
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Anan inside WSL2 to use the dashboard's /chat "
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/anan"`` (no trailing slash) or ``""`` when
...
def mount_spa(application: FastAPI):
    ...
    # (the MC Pages app), not the Anan backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Anan Teal",         "description": "Classic dark teal — the canonical Anan look"},
    {"name": "default-large", "label": "Anan Teal (Large)", "description": "Anan Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.anan/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.anan/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_anan_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.anan/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as anan_cli.plugins):
    1. User plugins:    ~/.anan/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.anan/plugins/  (only if SINOCLAW_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    search_dirs = [
        (get_anan_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    if os.environ.get("SINOCLAW_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".anan" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from anan_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_anan_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from anan_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from anan_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from anan_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from anan_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"anan_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    *,
    ...
    print(f"  Anan Web UI → http://{host}:{port}")
