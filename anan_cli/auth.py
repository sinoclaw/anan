"""
Multi-provider authentication system for Anan Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.anan/auth.json with cross-process file locking.
...
from anan_cli.config import get_anan_home, get_config_path, read_raw_config
...
DEFAULT_NOUS_CLIENT_ID = "anan-cli"
...
SPOTIFY_DOCS_URL = "https://anan.nousresearch.com/docs/user-guide/features/spotify"
...
def get_anthropic_key() -> str:
    ...
    """
    from anan_cli.config import get_env_value
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from anan_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.anan/.env file
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `anan model` to re-authenticate."
...
# =============================================================================
# Auth Store — persistence layer for ~/.anan/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_anan_home() / "auth.json"
    # Seat belt: if pytest is running and ANAN_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch ANAN_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".anan" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set ANAN_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom ANAN_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_anan_root()
    except Exception:
        return None
    profile_home = get_anan_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.anan/auth.json`` even when ANAN_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_anan_root()`` for a profile-shaped ANAN_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".anan" / "auth.json"
            try:
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``anan auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Anan.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `anan logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from anan_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'anan doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'anan model' for available providers, or run 'anan doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'anan model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.anan/.env.",
...
#
# Tokens live in ~/.anan/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `anan auth list` / `anan status`."""
    try:
...
# Spotify auth — PKCE tokens stored in ~/.anan/auth.json
# =============================================================================
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `anan auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `anan auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `anan auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `anan auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.anan/.env, and return it.
    ...
    print("       App name:     anything (e.g. anan)")
    ...
    # Persist so subsequent `anan auth spotify` runs skip the wizard.
    ...
    print()
    print("Saved SINOCLAW_SPOTIFY_CLIENT_ID to ~/.anan/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    print()
    print("Open this URL to authorize Anan:")
    print(authorize_url)
    print()
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.anan/auth.json (not ~/.codex/)
#
# Anan maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Anan auth store (~/.anan/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `anan auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `anan auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `anan auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `anan auth` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Anan auth store (~/.anan/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Anan auth state."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `anan auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Anan auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Anan's own Codex token store."""
    ...
    if should_refresh:
        # Re-read under lock to avoid racing with other Anan processes
...
# so a new `anan --profile <name> auth add nous --type oauth` can one-tap
...
def _nous_shared_auth_dir() -> Path:
    ...
    Docker / custom ``ANAN_HOME`` deployments at
    ``<ANAN_HOME>/shared/``. Sits outside any named profile so all
    ...
    from anan_constants import get_default_anan_root
    return get_default_anan_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Anan root, refuse rather than silently corrupt cross-profile
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from anan_constants import get_default_anan_root
        real_home_shared = (
            get_default_anan_root() / "shared" / NOUS_SHARED_STORE_FILENAME
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No ANAN_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with Anan's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Anan's next refresh uses it, and the whole
    ...
    if "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "custom self-heal hook, or another Anan install sharing "
            "~/.anan/auth.json) called POST /api/oauth/token with Anan's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only Anan may call the "
            "refresh endpoint. For health checks, use `anan auth status` "
            "instead.\n"
            "Re-authenticate with: anan auth add nous"
        )
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Anan models — they're not reliable for agentic tool-calling
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Anan is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``anan auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``anan auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `anan auth add nous --type oauth`. Best-
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Anan is not logged into Nous Portal.",
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `anan auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `anan auth` and
    # `anan model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `anan logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    """
    from anan_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from anan_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'anan model' or 'anan setup' instead."""
    print("The 'hermes login' command has been removed.")
    print("Use 'hermes auth' to manage credentials,")
    print("'anan model' to select a provider, or 'anan setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.anan/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Anan-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Anan auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Anan will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in ("y", "yes"):
                ...
                print("Anan will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Anan gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Anan creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Anan auth store
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Anan auth store (~/.anan/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Anan login via MiniMax ({region}) OAuth...")
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    if not state or not state.get("access_token"):
        raise AuthError(
            "Not logged into MiniMax OAuth. Run `anan model` and select "
            "MiniMax (OAuth).",
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Anan login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `anan model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Run `anan model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Anan will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `anan model` or configure an API key to use Anan.")
        else:
