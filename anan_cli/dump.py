"""
Dump command for anan CLI.

Outputs a compact, plain-text summary of the user's Anan setup
...
from anan_cli.config import get_anan_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``anan dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from anan_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(anan_home: Path) -> int:
...
def _cron_summary(anan_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = anan_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"anan_home:      {display_anan_home()}")
    ...
    toolsets = config.get("toolsets", ["anan-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(anan_home)}")
    lines.append(f"  skills:             {_count_skills(anan_home)}")
