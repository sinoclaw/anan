"""
Unified tool configuration for Anan Agent.

`anan tools` and `anan setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.anan/config.yaml under
...
from anan_cli.nous_subscription import (
...
from anan_cli.cli_output import (  # noqa: E402 — late import block
...
# Platform-scoped toolsets: only appear in the `anan tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``anan tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from anan_cli.plugins import discover_plugins, get_plugin_toolsets
...
from anan_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_anan_home()}/anan && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in anan directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/anan:latest"
            )
            return
        ...
        try:
            ...
            if result.returncode == 0 and shutil.which("cua-driver"):
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Anan process.")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.anan/config.yaml")
        ...
        # Run the full `anan auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.anan/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from anan_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: anan auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `anan auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: anan auth spotify")
        except Exception as exc:
        ...
        try:
            from anan_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: anan plugins enable observability/langfuse")
        _print_info("    Restart Anan for tracing to take effect.")
        _print_info("    Verify: anan plugins list")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "anan-cli" (which include all _SINOCLAW_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "anan-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # otherwise saving via `anan tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"anan-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("anan-")}
    ...
    # shipped as a bundled plugin but user must opt in via `anan tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `anan tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by anan tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (anan-cli, sinoclaw-telegram, etc.)
    ...
    # Opening `anan tools` is the user's opt-in to reconfigure tools, so treat
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `anan tools` and `anan setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Anan Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://anan.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from anan_constants import display_anan_home
    print(color(f"  Tool configuration saved to {display_anan_home()}/config.yaml", Colors.DIM))
