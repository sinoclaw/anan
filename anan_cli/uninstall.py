"""
Anan Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.anan/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Anan PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing anan or anan PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Anan Agent" comment and following line
                if '# Anan Agent' in line or '# anan' in line:
                    skip_next = True
                    continue
...
def remove_wrapper_script():
    """Remove the anan wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "hermes",
        Path("/usr/local/bin/anan"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains anan_cli reference)
                content = wrapper.read_text()
                if 'anan_cli' in content or 'anan' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``anan gateway run`` processes
    ...
    try:
        from anan_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from anan_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ANAN_HOME`` and ``SINOCLAW_GIT_BASH_PATH``
...
#   4. On the ``anan dashboard`` + gateway paths, drops files into
...
def _anan_path_markers(anan_home: Path) -> list[str]:
    """Path-entry substrings that identify Anan-owned User-PATH entries."""
    root = str(anan_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare anan install dir.
    markers = [root + "\\anan", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ANAN_HOME was customised to somewhere else — find-and-nuke
...
def remove_path_from_windows_registry(anan_home: Path) -> list[str]:
    """Strip Anan-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _anan_path_markers(anan_home)
            kept: list[str] = []
...
def remove_anan_env_vars_windows() -> list[str]:
    """Delete ANAN_HOME and SINOCLAW_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ANAN_HOME", "SINOCLAW_GIT_BASH_PATH"):
                try:
...
def remove_portable_tooling_windows(anan_home: Path) -> list[Path]:
...
def _is_default_anan_home(anan_home: Path) -> bool:
    """Return True when ``anan_home`` points at the default (non-profile) root."""
    try:
        from anan_constants import get_default_anan_root
        return anan_home.resolve() == get_default_anan_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ANAN_HOME directory.

    We shell out to ``anan -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ANAN_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m anan_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    anan_invocation = [_sys.executable, "-m", "anan_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                anan_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ANAN_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.anan/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.anan/ for future reinstall
    """
    project_root = get_project_root()
    anan_home = get_anan_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ANAN_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_anan_home(anan_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Anan Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {anan_home / 'config.yaml'}")
    print(f"  Secrets: {anan_home / '.env'}")
    print(f"  Data:    {anan_home / 'cron/'}, {anan_home / 'sessions/'}, {anan_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ANAN_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Anan data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Anan code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in anan_home so the marker matching is
        ...
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(anan_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Anan-owned PATH entries in User environment")

        log_info("Removing ANAN_HOME / SINOCLAW_GIT_BASH_PATH User env vars...")
        removed_env = remove_anan_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Anan-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing anan command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.anan/, just remove the anan subdir
            if anan_home in project_root.parents or project_root.parent == anan_home:
    ...
    #     under ANAN_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(anan_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(anan_home)
    ...
    # 5. Optionally remove ~/.anan/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ANAN_HOME dirs live under
        ...
        try:
            if anan_home.exists():
                shutil.rmtree(anan_home)
                log_success(f"Removed {anan_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {anan_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {anan_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  irm https://raw.githubusercontent.com/anan/anan/main/scripts/install.ps1 | iex", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/anan/anan/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Anan Agent! ⚕")
    print()
