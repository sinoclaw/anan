"""
MCP Server Management CLI — ``anan mcp`` subcommand.

Implements ``anan mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.anan/config.yaml under the ``mcp_servers`` key.
"""
...
from anan_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_anan_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from anan_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    # anan_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  anan mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  anan mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  anan mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_anan_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: anan mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_anan_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `anan mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  anan mcp add <name> --url <endpoint>')
        _info('  anan mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `anan mcp remove` + `anan mcp add` to reconfigure auth.")
        return
...
def mcp_command(args):
    """Main dispatcher for ``anan mcp`` subcommands."""
