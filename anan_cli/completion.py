"""Shell completion script generation for anan CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_anan_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Anan Agent bash completion
# Add to ~/.bashrc:
...
_hermes() {{
    ...
    _arguments -C \\
        ...
        '(-p --profile){{-p,--profile}}[Profile name]:profile:_anan_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Anan Agent fish completion",
        "# Add to your config:",
        "#   anan completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __anan_profiles",
        "    echo default",
        "    if test -d $HOME/.anan/profiles",
        "        ls $HOME/.anan/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c anan -f -s p -l profile"
        " -d 'Profile name' -xa '(__anan_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__anan_profiles)' -d 'Profile name'"
                )
