...
Usage::
    ...
    anan hooks list
    anan hooks test <event> [--for-tool X] [--payload-file F]
    anan hooks revoke <command>
    anan hooks doctor

Consent records live under ``~/.anan/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.anan/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``anan hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: anan hooks {list|test|revoke|doctor}")
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.anan/config.yaml.")
        print("See `anan hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `anan hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `anan hooks test` and `anan hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Anan wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `anan hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `anan hooks doctor` would execute
