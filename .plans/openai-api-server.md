# OpenAI-Compatible API Server for Anan Agent
...
SSE streaming. By exposing this endpoint, anan becomes instantly
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.anan/.env`
2. Run `anan gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with anan through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns anan as a model) |
...
But anan sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "anan"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured anan model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of anan's core prompt
...
Once implemented, anan works as a drop-in backend for:
