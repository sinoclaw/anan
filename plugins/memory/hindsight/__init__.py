...
Or via $ANAN_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so anan can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $ANAN_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_anan_home() / "hindsight" / "config.json"
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this anan config."""
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active anan profile name (from agent_identity)
      {workspace} — anan workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``anan-{user}`` with no user becomes ``anan``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between anan Agent and the User"
    ...
    def save_config(self, values, anan_home):
        """Write config to $ANAN_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(anan_home) / "hindsight"
    ...
    def post_setup(self, anan_home: str, config: dict) -> None:
        ...
        from anan_cli.memory_setup import _curses_select
        ...
        self.save_config(provider_config, anan_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(anan_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(anan_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: anan-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between anan Agent and the User"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._retain_context = self._config.get("retain_context", "conversation between anan Agent and the User")
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When anan created/used that client on the shared async loop,
