...
|-----|---------|-------------|
| `container_tag` | `anan` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `anan-{identity}` → `anan-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Anan profile:
