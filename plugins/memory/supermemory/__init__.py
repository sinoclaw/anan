...
def _load_supermemory_config(anan_home: str) -> dict:
    config = _default_config()
    config_path = Path(anan_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, anan_home: str) -> None:
    config_path = Path(anan_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `anan memory setup`.
        # All other options are documented for $ANAN_HOME/supermemory.json
    ...
    def save_config(self, values, anan_home):
        ...
        _save_supermemory_config(sanitized, anan_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from anan_constants import get_anan_home
        self._anan_home = kwargs.get("anan_home") or str(get_anan_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._anan_home)
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "anan_memory", "target": target, "type": "explicit_memory"},
