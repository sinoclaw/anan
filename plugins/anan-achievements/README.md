...
>
> **Share cards (2026-05-04, vendored in anan v0.4.0):** Unlocked achievement cards now have a "Share" button that renders a 1200×630 PNG share card (client-side canvas, no backend, no network) with Download + Copy-to-clipboard actions. Fits X/Twitter, Discord, LinkedIn, Bluesky link-preview dimensions.
...
- Anan-native skills, memory, cron, and plugin usage
...
```bash
git clone https://github.com/PCinkusz/anan-achievements ~/anan-achievements
ln -s ~/anan-achievements ~/.anan/plugins/anan-achievements
```
...
If backend API routes 404, restart `anan dashboard`; plugin APIs are mounted at dashboard startup.
...
```bash
cd ~/.anan/plugins/anan-achievements
git pull --ff-only
curl http://127.0.0.1:9119/api/dashboard/plugins/rescan
```

If the update changes backend routes or `plugin_api.py`, restart `anan dashboard` after pulling.
...
Achievement unlock state is stored locally in `state.json` and is not overwritten by git updates. New achievements are evaluated from your existing Anan session history. Achievement IDs are stable and should not be renamed casually because they are the unlock-state keys.
