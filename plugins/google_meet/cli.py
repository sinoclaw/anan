...
def _auth_state_path() -> Path:
    return Path(get_anan_home()) / "workspace" / "meetings" / "auth.json"
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    """Build the ``anan meet`` argparse tree.
    ...
    join_p.add_argument("--guest-name", default="Anan Agent")
...
def meet_command(args: argparse.Namespace) -> int:
    ...
    if not sub:
        print("usage: anan meet {setup,auth,join,status,transcript,say,stop,node}")
        return 2
    if sub == "setup":
        return _cmd_setup()
    if sub == "install":
    ...
    if sub == "node":
        ...
        if fn is None or fn is meet_command:
            print("usage: anan meet node {run,list,approve,remove,status,ping}")
            return 2
        return fn(args)
...
def _cmd_setup() -> int:
    ...
    print(
        "  google auth    : "
        + (f"ok ({auth_path})" if auth_ok else "not saved — run: anan meet auth")
    )
    ...
    if realtime:
        ...
        if system == "Linux":
            ...
            print(
                ...
                "  anan will not switch your default input for you."
            )
    else:
        print("\n[3/3] skipped (pass --realtime to install audio tooling too)")

    print("\ndone. verify with: anan meet setup")
    return 0
...
def _cmd_auth() -> int:
    ...
    try:
        ...
        return 1
    print("saved. you can now run: anan meet join <url>")
    return 0
