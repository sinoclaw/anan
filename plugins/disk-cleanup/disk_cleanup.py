"""disk_cleanup — ephemeral file cleanup for Anan Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ANAN_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ANAN_HOME and /tmp/anan-*
Never touches: ~/.anan/logs/ or any system directory.
"""
...
try:
    ...
    def get_anan_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ANAN_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".anan").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ANAN_HOME/logs/``."""
    return get_anan_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ANAN_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ANAN_HOME or ``/tmp/anan-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    anan_home = get_anan_home()
    try:
        path.resolve().relative_to(anan_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/anan-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("anan-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ANAN_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ANAN_HOME (but leave ANAN_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "anan", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(anan_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == anan_home:
                continue
            try:
                rel_parts = dirpath.relative_to(anan_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    anan_home = get_anan_home()
    try:
        rel = path.resolve().relative_to(anan_home)
        ...
        # Path isn't under ANAN_HOME (e.g. /tmp/anan-*) — fall through.
        pass
