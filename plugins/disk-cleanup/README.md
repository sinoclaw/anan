...
Auto-tracks and cleans up ephemeral files created during Anan Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ANAN_HOME` and `/tmp/anan-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ANAN_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ANAN_HOME` or `/tmp/anan-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ANAN_HOME/disk-cleanup/` is itself excluded
- `$ANAN_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
