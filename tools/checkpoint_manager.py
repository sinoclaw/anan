...
CHECKPOINT_BASE = get_anan_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/anan"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Anan convention — don't recursively snapshot siblings)
    ".worktrees/",
...
def _git_env(
    ...
    The shared store is internal Anan infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``anan checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `anan checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.name", "Anan Checkpoint"], store, cfg_wd)
...
# ---------------------------------------------------------------------------
# Public helpers for `anan checkpoints` CLI
# ---------------------------------------------------------------------------
