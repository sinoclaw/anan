...
def _is_anan_provider_credential(name: str) -> bool:
    """True if ``name`` is a Anan-managed provider credential (API key,
    ...
    Non-Anan API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Anan-managed provider credentials (from
    ...
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Anan-managed
    ...
    Non-Anan third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_anan_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Anan provider "
