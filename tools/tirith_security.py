...
it is automatically downloaded from GitHub releases to $ANAN_HOME/bin/tirith.
...
def _get_anan_home() -> str:
    """Return the Anan home directory, respecting ANAN_HOME env var."""
    return str(get_anan_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_anan_home(), ".tirith-install-failed")
...
def _anan_bin_dir() -> str:
    """Return $ANAN_HOME/bin, creating it if needed."""
    d = os.path.join(_get_anan_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ANAN_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_anan_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ANAN_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ANAN_HOME/bin/tirith
    ...
    anan_bin = os.path.join(_anan_bin_dir(), "tirith")
    if os.path.isfile(anan_bin) and os.access(anan_bin, os.X_OK):
        _resolved_path = anan_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return anan_bin
