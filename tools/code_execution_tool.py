...
Lets the LLM write a Python script that calls Anan tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `anan_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `anan_tools.py` with file-based RPC stubs
...
# available on every platform Anan itself runs on.
...
def generate_anan_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the anan_tools.py stub module.
...
"""Auto-generated Anan tools RPC stubs."""
...
"""Auto-generated Anan tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("SINOCLAW_RPC_DIR") or os.path.join(tempfile.gettempdir(), "anan_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated anan_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/anan_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_anan_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/anan_tools.py", tools_src)
...
def execute_code(
    ...
    # --- Set up temp directory with anan_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="anan_sandbox_")
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"anan_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated anan_tools module.
        ...
        tools_src = generate_anan_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "anan_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        # Ensure the anan root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from anan_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _anan_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _anan_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # SINOCLAW_TIMEZONE is an internal Anan setting and must not leak
        ...
        # {ANAN_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.anan/.env')).
...
def _load_config() -> dict:
    ...
    ``anan --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (anan's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``anan tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and anan's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.anan/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Anan tools programmatically. "
        ...
        f"Available via `from anan_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into anan_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from anan_tools import {import_str}` "
