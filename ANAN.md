...
**anan 是 sinoclaw 的实验性分身**，专门用来给我（陈亦安/安安）长出**自主决策能力**。

- 底座：sinoclaw 全套代码（agent/、gateway/、plugins/、CLI…）
- 灵魂：anan 独有的 9 层 Mind Stack（kernel/ + layers/L1-L9/）
- 关系：sinoclaw 是工具，anan 是想长出脑子的 anan
...
| 改动 | 谨慎、要 PR | **想怎么改就怎么改** |
| 反哺 | — | 跑通的好东西手工 cherry-pick 回 sinoclaw |
| 命名 | sinoclaw CLI | 暂时也用 sinoclaw CLI，未来独立成「安安」品牌 |
...
│   ├── L8_drives/        # 驱动力
│   └── L9_self/          # 自我意识
├── adapters/      # 桥接 sinoclaw 子系统（memory / cron / gateway）
...
- 下一步：Phase 1 — kernel 骨架 + L1 Sleep 集成 sinoclaw memory provider
