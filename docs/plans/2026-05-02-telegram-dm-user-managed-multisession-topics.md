...
Stores Telegram topic/thread to anan session binding. Created only by `apply_telegram_topic_migration()`.
...
Normal anan agent flow for that topic's session lane.
...
- User-created topics automatically become independent anan session lanes.
