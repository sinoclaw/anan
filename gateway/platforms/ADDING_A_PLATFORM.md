...
There are two ways to add a platform to the Anan gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.anan/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Anan code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `anan gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `anan_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Anan core.
...
And add it to the `anan-gateway` composite:

```python
"anan-gateway": {
    "includes": [..., "anan-your-platform"]
}
```
...
## 12. Status Display (`anan_cli/status.py`)
...
## 13. Gateway Setup Wizard (`anan_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ anan_cli/ toolsets.py \
  --include="*.py" -l | sort -u
