...
normal tests assume away:
  ...
  - Filesystem: ANAN_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own ANAN_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            ...
            for m in list(sys.modules.keys()):
                if m.startswith(("anan_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from anan_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "ANAN_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
    ...
    for bad in bad_metas:
        r = subprocess.run(
            [sys.executable, "-m", "anan_cli.main", "kanban",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from anan_cli.kanban_db import resolve_workspace
            ...
            home_abs = str(Path(os.environ["ANAN_HOME"]).resolve())
            if not resolved_abs.startswith(home_abs) and resolved_abs.startswith("/tmp"):
                ...
                print(f"  ⚠ workspace resolved OUTSIDE anan_home: {resolved}")
...
@scenario("anan_home_with_spaces")
def _(home, kb):
    """ANAN_HOME at a path with spaces — should work but catches
    ...
    try:
        ...
        print(f"  ANAN_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("anan_home_with_unicode")
def _(home, kb):
    ...
    try:
        ...
        print(f"  ANAN_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("anan_home_via_symlink")
def _(home, kb):
    ...
    try:
        ...
        conn2.close()
        print("  symlinks to same ANAN_HOME share DB correctly")
    finally:
...
def _idempotency_race_worker(anan_home: str, key: str, result_file: str,
...
def _(home, kb):
    ...
    try:
        from anan_cli import web_server as ws  # noqa
    except Exception:
        pass
