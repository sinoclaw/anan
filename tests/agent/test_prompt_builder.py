...
class TestBuildNousSubscriptionPrompt:
    def test_includes_active_subscription_features(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "anan_cli.nous_subscription.get_nous_subscription_features",
...
class TestBuildContextFilesPrompt:
    ...
    def test_loads_soul_md_from_anan_home_only(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ANAN_HOME", str(tmp_path / "anan_home"))
        anan_home = tmp_path / "anan_home"
        anan_home.mkdir()
        (anan_home / "SOUL.md").write_text("Be concise and friendly.", encoding="utf-8")
    ...
    def test_anan_md_parent_dir_discovery(self, tmp_path):
        ...
        (tmp_path / ".git").mkdir()
        (tmp_path / ".anan.md").write_text("Root project rules.")
...
# =========================================================================
# .anan.md helper functions
# =========================================================================


class TestFindHermesMd:
    def test_finds_in_cwd(self, tmp_path):
        (tmp_path / ".anan.md").write_text("rules")
        assert _find_anan_md(tmp_path) == tmp_path / ".anan.md"
    ...
    def test_prefers_lowercase(self, tmp_path):
        (tmp_path / ".anan.md").write_text("lower")
        (tmp_path / "ANAN.md").write_text("upper")
        assert _find_anan_md(tmp_path) == tmp_path / ".anan.md"

    def test_walks_to_git_root(self, tmp_path):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".anan.md").write_text("root rules")
        sub = tmp_path / "a" / "b"
        sub.mkdir(parents=True)
        assert _find_anan_md(sub) == tmp_path / ".anan.md"

    def test_returns_none_when_absent(self, tmp_path):
        assert _find_anan_md(tmp_path) is None

    def test_stops_at_git_root(self, tmp_path):
        """Does not walk past the git root."""
        (tmp_path / ".anan.md").write_text("outside")
        repo = tmp_path / "repo"
        repo.mkdir()
        (repo / ".git").mkdir()
        assert _find_anan_md(repo) is None
