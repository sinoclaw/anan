...
def test_persist_dm_topic_thread_id_writes_config(tmp_path):
    ...
    config_file = tmp_path / ".anan" / "config.yaml"
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"ANAN_HOME": str(tmp_path / ".anan")}):
...
def test_group_topic_chat_id_int_string_coercion():
    ...
    adapter = _make_adapter(group_topics_config=[
        {
            ...
            "topics": [
                {"name": "Dev", "thread_id": "7", "skill": "anan-dev"},
            ],
        }
    ])
    ...
    assert event.auto_skill == "anan-dev"
