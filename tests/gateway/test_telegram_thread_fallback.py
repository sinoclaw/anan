...
Supergroup forum topics route with ``message_thread_id``. anan-created
...
async def test_send_typing_skips_api_call_for_dm_topic_reply_fallback():
    """anan-created DM topic lanes have no working Bot API typing route.
...
async def test_gateway_runner_busy_ack_replies_to_triggering_message_for_telegram_dm_topic(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway_run, "_anan_home", tmp_path)
...
async def test_send_uses_metadata_reply_fallback_for_streaming_dm_topics():
    """Metadata-only sends still stay in anan-created Telegram DM topics."""
    adapter = _make_adapter()
    call_log = []
