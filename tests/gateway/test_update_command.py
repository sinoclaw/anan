...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        ...
        assert "brew upgrade anan" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _anan_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._anan_home", tmp_path), \
        ...
        with patch("gateway.run._anan_home", tmp_path):
            ...
            # project_root will be the real anan dir (which HAS .git).
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        assert not (anan_home / ".update_exit_code").exists()
    ...
    async def test_writes_pending_marker_with_thread_id(self, tmp_path):
        ...
        data = json.loads((anan_home / ".update_pending.json").read_text())
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (anan_home / ".update_pending.json").exists()
        assert not (anan_home / ".update_exit_code").exists()
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (anan_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = anan_home / ".update_pending.claimed.json"
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (anan_home / ".update_pending.json").write_text(json.dumps(pending))
        (anan_home / ".update_output.txt").write_text("x" * 5000)
        (anan_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (anan_home / ".update_pending.json").write_text(json.dumps(pending))
        (anan_home / ".update_output.txt").write_text("Traceback: boom")
