...
def test_ws_events_rejects_when_token_required(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "anan_cli.web_server", stub)
    monkeypatch.setattr(anan_cli, "web_server", stub, raising=False)
...
def test_create_task_probe_error_does_not_break_create(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "anan_cli.kanban._check_dispatcher_presence", _raise,
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
def test_board_surfaces_warnings_field_for_hallucinated_completions(client):
    ...
    raw event kind — see anan_cli.kanban_diagnostics for the rule
    that produces it.
    """
    conn = kb.connect()
    try:
