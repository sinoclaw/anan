...
def load_module():
    spec = importlib.util.spec_from_file_location("openclaw_to_anan", SCRIPT_PATH)
...
"""
    ...
    assert "Tyler Williams > Active Projects: anan Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `anan cron` to recreate scheduled tasks" in notes_text
...
def test_rebrand_text_replaces_openclaw_variants():
    ...
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "anan prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told anan to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "anan config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "anan uses tools well"
    # All-lowercase matches → lowercase ``anan``; this preserves the
    # real filesystem path ``~/.anan`` (anan home) when rebranding
    # memory entries that reference ``~/.anan`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "anan should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "anan remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "anan prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "anan was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "anan uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "anan said to ask anan about anan settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.anan`` (the real anan home), not the broken
    ``.Hermes``.
    ...
    assert mod.rebrand_text("use .openclaw directory") == "use .anan directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.anan'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.anan/") == \
        "anan config path: ~/.anan/"
