...
Key invariants covered by these tests:
  ...
  5. Load order: every Anan entry point imports anan_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported anan_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("anan_bootstrap", None)
    import anan_bootstrap  # noqa: WPS433
    return anan_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Anan CLI
...
class TestEntryPointsImportBootstrap:
    """Every Anan entry point must import anan_bootstrap as its
    ...
    # Entry points that invoke Anan as a process.  Each one must
    # import anan_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import anan_bootstrap' and that
        ...
        half-finished ``anan update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``anan_bootstrap`` as a top-level module) leaves hermes
        ...
        """
        # Resolve relative to the anan repo root.  Tests live
        # at tests/test_anan_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # anan start even when anan_bootstrap hasn't been
        ...
        assert first_import_name == "anan_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'anan_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import anan_bootstrap' line to be the first import."
        )
