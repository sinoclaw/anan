...
# ---------------------------------------------------------------------------
# Toolset: discord / discord_admin only in anan-discord
# ---------------------------------------------------------------------------

class TestToolsetInclusion:
    def test_discord_tools_in_anan_discord_toolset(self):
        from toolsets import TOOLSETS
        assert "discord" in TOOLSETS["anan-discord"]["tools"]
        assert "discord_admin" in TOOLSETS["anan-discord"]["tools"]
    ...
    def test_discord_tools_not_in_other_toolsets(self):
        ...
        for name, ts in TOOLSETS.items():
            if name in ("anan-discord", "anan-gateway", "discord", "discord_admin"):
                continue
            tools = ts.get("tools", [])
...
class TestConfigAllowlist:
    ...
    def test_config_load_failure_is_permissive(self, monkeypatch):
        ...
        monkeypatch.setattr("anan_cli.config.load_config", bad_load)
        assert _load_allowed_actions_config() is None
...
class TestModelToolsIntegration:
    ...
    def test_discord_admin_schema_rebuilt_by_get_tool_definitions(
        ...
        tools = get_tool_definitions(enabled_toolsets=["anan-discord"], quiet_mode=True)
