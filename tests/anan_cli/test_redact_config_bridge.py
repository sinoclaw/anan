...
`SINOCLAW_REDACT_SECRETS` at module-import time. `anan_cli/main.py` at
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports anan_cli.main and checks
    ...
    probe = textwrap.dedent(
        ...
        sys.path.insert(0, %r)
        import anan_cli.main  # triggers the bridge + setup_logging
        import agent.redact
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    ...
    (anan_home / ".env").write_text("SINOCLAW_REDACT_SECRETS=true\n")
