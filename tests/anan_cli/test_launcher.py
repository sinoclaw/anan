...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `anan_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "anan"
    called = []

    fake_main_module = types.ModuleType("anan_cli.main")

    def fake_main():
        called.append("anan_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "anan_cli.main", fake_main_module)
