...
- Bare `anan setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `anan setup --quick` runs the narrower "fill in missing items" flow.
- `anan setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("anan_cli.setup.ensure_anan_home", {}),
        ("anan_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("anan_cli.config.is_managed", {"return_value": False}),
        ("anan_cli.setup.load_config", {"return_value": {}}),
        ("anan_cli.setup.save_config", {}),
        ("anan_cli.setup.get_env_value", {"return_value": None}),
        ("anan_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("anan_cli.setup._print_setup_summary", {}),
        ("anan_cli.setup._offer_launch_chat", {}),
        ("anan_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("anan_cli.setup.load_config", {"return_value": {}}),
        ("anan_cli.setup.save_config", {}),
        ("anan_cli.auth.get_active_provider", {"return_value": None}),
        ("anan_cli.setup.get_env_value", {"return_value": None}),
        ("anan_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `anan setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="anan_cli.setup.prompt_choice",
                quick="anan_cli.setup._run_quick_setup",
                model="anan_cli.setup.setup_model_provider",
                terminal="anan_cli.setup.setup_terminal_backend",
                agent="anan_cli.setup.setup_agent_settings",
                gateway="anan_cli.setup.setup_gateway",
                tools="anan_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`anan setup --reconfigure` behaves the same as bare `anan setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("anan_cli.setup.prompt_choice", {"return_value": 0}),
                first="anan_cli.setup._run_first_time_quick_setup",
            )
            from anan_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        import sys
        from anan_cli.main import main

        captured = {}
        monkeypatch.setattr(
            "anan_cli.setup.run_setup_wizard",
