...
import anan_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_stop_timeout_prints_status_guidance(self, monkeypatch, capsys):
        ...
        assert "anan gateway status" in output

    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`anan gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `anan gateway restart` in the background; when a
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `anan gateway restart`)."""
        unit_path = tmp_path / "anan-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "anan gateway install" in out
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_anan_home_for_target_user", lambda home: "/home/alice/.anan")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestGatewaySystemServiceRouting:
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: anan gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup anan gateway" in out
...
class TestSystemUnitHermesHome:
    """ANAN_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'ANAN_HOME=/home/alice/.anan' in unit
        assert '/root/.anan' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: ANAN_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("ANAN_HOME", "/root/.anan/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'ANAN_HOME=/home/alice/.anan/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_anan_home(self, monkeypatch):
        # Custom ANAN_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("ANAN_HOME", "/opt/anan-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'ANAN_HOME=/opt/anan-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's ANAN_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        anan_home = str(gateway_cli.get_anan_home().resolve())
        assert f'ANAN_HOME={anan_home}' in unit


class TestSinoclawHomeForTargetUser:
    """Unit tests for _anan_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("ANAN_HOME", raising=False)

        result = gateway_cli._anan_home_for_target_user("/home/alice")
        assert result == "/home/alice/.anan"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._anan_home_for_target_user("/home/alice")
        assert result == "/home/alice/.anan/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("ANAN_HOME", "/opt/anan")

        result = gateway_cli._anan_home_for_target_user("/home/alice")
        assert result == "/opt/anan"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "anan gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_default_anan_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.anan should not produce a --profile flag."""
        anan_home = tmp_path / ".anan"
        anan_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANAN_HOME", str(anan_home))
        result = gateway_cli._profile_arg(str(anan_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.anan/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".anan" / "profiles" / "mybot"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile ANAN_HOME should return empty string."""
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.anan/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".anan" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".anan" / "profiles" / "My Bot!"
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.anan.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".anan" / "anan"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".anan" / "anan")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/anan", str(tmp_path / "alice"))
        assert result == "/opt/anan"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".anan" / "anan")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("ANAN_HOME", str(root_home / ".anan"))
        monkeypatch.setattr(gateway_cli, "get_anan_home", lambda: root_home / ".anan")
        ...
        assert "WorkingDirectory=/home/alice/.anan/anan" in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "anan-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "anan-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'anan gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'anan gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'anan gateway start' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyHermesUnitDetection:
    ...
    renamed to ``anan-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``anan.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``anan-gateway-<profile>.service``) or unrelated third-party services.
    """
    ...
    def test_ignores_profile_unit_sinoclaw_gateway_coder(self, tmp_path, monkeypatch):
        ...
        for base in (user_dir, system_dir):
            (base / "anan-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "anan-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "anan-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
    ...
    def test_ignores_unrelated_sinoclaw_service(self, tmp_path, monkeypatch):
        ...
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "anan.service").write_text(
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m anan_cli.main gateway run
          - python path/to/anan_cli/main.py gateway run
          - anan gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m anan_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hermes/anan_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/anan gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hermes/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = f"anan.service" if i == 0 else f"anan.service"  # same name
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "anan.service" in out
        assert "anan gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "anan.service"
...
class TestRemoveLegacyHermesUnits:
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop anan.service" in c for c in cmds_joined)
        assert any("--user disable anan.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo anan gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop anan.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable anan.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "anan.service"
        system_legacy = system_dir / "anan.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (anan-gateway-coder.service)
        ...
        profile_unit = user_dir / "anan-gateway-coder.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `anan gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import anan_cli.main as cli_main
        ...
        # public, just confirm that `anan gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import anan_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "anan_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "anan_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "anan-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "anan-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "anan-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``anan gateway start --system``).
        self._setup_units(tmp_path, monkeypatch, system_present=False, user_present=False)
        monkeypatch.setattr(gateway_cli.os, "geteuid", lambda: 1000)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "anan-gateway")
        ...
        assert "sudo systemctl start anan-gateway" in out
        assert "sudo anan gateway uninstall --system" in out
        assert "anan gateway install" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart anan-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop anan-gateway" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``anan gateway start --system`` etc.) must
