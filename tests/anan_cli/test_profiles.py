"""Comprehensive tests for anan_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and ANAN_HOME for profile tests
# ---------------------------------------------------------------------------
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes anan, .worktrees,
        ...
        (default_home / "anan" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "anan").exists()
...
class TestNoSkillsOptOut:
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `anan update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("anan_cli.profiles._cleanup_gateway_service"):
...
class TestGetActiveProfileName:
    ...
    def test_default_anan_home_returns_default(self, profile_env):
        # ANAN_HOME points to tmp_path/.hermes which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom ANAN_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("ANAN_HOME", str(custom))
        # With Docker-aware roots, a custom ANAN_HOME is the default —
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        with patch("anan_cli.profiles.check_alias_collision", return_value="skip"):
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("anan", ".worktrees", "profiles", "bin",
        ...
        excluded_prefixes = [
            "default/anan", "default/.worktrees", "default/profiles",
...
class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_anan_home()."""
    ...
    def test_default_anan_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_anan_home() returns ANAN_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANAN_HOME", str(docker_home))
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under ANAN_HOME."""
        from anan_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANAN_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under ANAN_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ANAN_HOME", str(docker_home))
