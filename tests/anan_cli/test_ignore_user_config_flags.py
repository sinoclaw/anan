"""Tests for --ignore-user-config and --ignore-rules flags on `anan chat`.
...
files. In anan the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.anan/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → AnanCLI → AIAgent call chain.
"""
...
class TestIgnoreUserConfigEnvGate:
    ...
    When the env var is set, user config at ``<anan_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._anan_home at tmp_path and return a fresh load_cli_config."""
        import cli
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``AnanCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting SINOCLAW_IGNORE_RULES=1 flips AnanCLI.ignore_rules True."""
        monkeypatch.setenv("SINOCLAW_IGNORE_RULES", "1")

        # Import AnanCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of AnanCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.AnanCLI)
        # Replicate the exact logic from cli.py AnanCLI.__init__:
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``anan_cli/main.py`` must set
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real anan parser accepts both flags."""
