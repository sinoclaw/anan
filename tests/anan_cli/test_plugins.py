...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<anan_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<anan_home>/plugins/``; we derive
    ``<anan_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <ANAN_HOME>/config.yaml.
        # Config is always read from ANAN_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        anan_home_str = os.environ.get("ANAN_HOME")
        if anan_home_str:
            anan_home = Path(anan_home_str)
        else:
            anan_home = base.parent
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("anan_cli.plugins._plugin_manager", mgr):
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("anan_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("anan_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("anan_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("anan_cli.plugins.PluginContext.dispatch_tool.__module__", "anan_cli.plugins"):
