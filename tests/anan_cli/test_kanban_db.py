"""Tests for the Kanban DB layer (anan_cli.kanban_db)."""
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="anan-task", assignee="daily")
...
def test_scratch_workspace_created_under_anan_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
class TestSharedBoardPaths:
    ...
    must anchor at the **shared root**, not the active profile's ANAN_HOME."""

    def _set_home(self, monkeypatch, tmp_path, anan_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.anan/kanban.db,
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # Worker switches to the profile ANAN_HOME and reads.
        monkeypatch.setenv("ANAN_HOME", str(profile_home))
        with kb.connect() as conn:
