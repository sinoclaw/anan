...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("anan_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_returns_anan_auth_store_source(tmp_path, monkeypatch):
    ...
    assert creds["source"] == "anan-auth-store"
...
def _patch_httpx(monkeypatch, response):
    ...
    monkeypatch.setattr("anan_cli.auth.httpx.Client", _factory)
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr("anan_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("anan_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
