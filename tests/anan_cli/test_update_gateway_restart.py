...
Ensures ``anan update`` correctly detects running gateways managed by
...
from anan_cli.main import cmd_update
...
def _no_restart_verify_sleep(monkeypatch):
    """anan_cli/main.py uses time.sleep(3) after systemctl restart to
...
def _make_run_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        # systemctl list-units anan-gateway* — discover all gateway services
...
class TestLaunchdPlistPath:
    def test_plist_contains_environment_variables(self):
        ...
        assert "<key>ANAN_HOME</key>" in plist
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_with_systemd_still_restarts_via_systemd(
        ...
        captured = capsys.readouterr().out
        assert "Restarted anan-gateway" in captured
        ...
        def side_effect(cmd, **kwargs):
            ...
            if "systemctl" in joined and "list-units" in joined:
                if "--user" in joined:
                    return subprocess.CompletedProcess(
                        cmd, 0,
                        stdout="anan-gateway.service loaded active running\n",
                        stderr="",
                    )
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
    ...
    def test_update_falls_back_to_systemctl_restart_when_sigusr1_times_out(
        ...
        monkeypatch.setattr(
            "anan_cli.gateway._graceful_restart_via_sigusr1",
    ...
    def test_update_bypasses_restartsec_after_graceful_drain(
        ...
        # Must have called ``reset-failed anan-gateway`` AND ``start
        # anan-gateway`` explicitly so systemd bypasses RestartSec.
        reset_calls = [c for c in calls if "reset-failed" in c and "anan-gateway" in c]
        start_calls = [
            c for c in calls
            if "start" in c and "anan-gateway" in c and "restart" not in c
        ]
        assert reset_calls, (
            f"Expected explicit `reset-failed anan-gateway` after graceful drain; "
            f"systemctl calls were: {calls}"
        )
        assert start_calls, (
            f"Expected explicit `start anan-gateway` after graceful drain to "
...
class TestServicePidExclusion:
    ...
    where ``anan update`` would restart the gateway and immediately kill it.
    """
...
class TestFindGatewayPidsExclude:
    ...
    def test_filters_to_current_profile(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(gateway_cli, "_profile_arg", lambda anan_home=None: "--profile orcha")
...
class TestGatewayModeWritesExitCodeEarly:
    """When running as ``anan update --gateway``, the exit code marker must be
    ...
    def test_exit_code_written_in_gateway_mode(
        ...
        monkeypatch.setattr(_cfg, "get_anan_home", lambda: anan_home)
        # Also patch the module-level ref used by cmd_update
        import anan_cli.main as _main_mod
        monkeypatch.setattr(_main_mod, "get_anan_home", lambda: anan_home)
        ...
        exit_code_path = anan_home / ".update_exit_code"
...
class TestCmdUpdateLegacyGatewayWarning:
    ...
    def test_update_does_not_flag_profile_units(
        ...
        (user_dir / "anan-gateway-coder.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        (user_dir / "anan-gateway.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
...
class TestCmdUpdateResetFailedBeforeRestart:
    """`anan update` must call `systemctl reset-failed` before every
    ...
    def test_reset_failed_runs_before_fallback_restart(
        ...
        assert any(
            "anan-gateway" in " ".join(str(c) for c in call)
            for call in reset_calls
        ), (
            "Expected `systemctl reset-failed anan-gateway` before the "
        ...
        for idx, call in enumerate(mock_run.call_args_list):
            ...
            if "systemctl" in joined and "reset-failed" in joined and first_reset_idx is None:
                first_reset_idx = idx
            if "systemctl" in joined and "restart" in joined and "anan-gateway" in joined:
    ...
    def test_reset_failed_also_runs_before_retry_restart(
        ...
        gateway_restarts = [
            ...
            if "anan-gateway" in " ".join(str(a) for a in c)
        ]
        gateway_resets = [
    ...
    def test_final_failure_message_tells_user_to_reset_failed(
        ...
        assert "anan-gateway" in captured
