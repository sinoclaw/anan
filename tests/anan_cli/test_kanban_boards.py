"""Tests for the multi-board kanban layer (``anan kanban boards …``).
...
* CLI surface: ``anan kanban boards list/create/switch/rm``.
...
def fresh_home(tmp_path, monkeypatch):
    """Isolated ANAN_HOME with no prior kanban state.

    The autouse hermetic conftest already nukes credentials + TZ; this
    fixture layers a per-test ANAN_HOME plus a path-init cache reset
    so each test sees a truly empty board set.
    """
    home = tmp_path / "anan_home"
    home.mkdir()
...
class TestSlugValidation:
    @pytest.mark.parametrize("good", [
        "default", "atm10-server", "anan", "proj_1", "a",
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``anan kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
    env["PYTHONPATH"] = str(_WORKTREE)
    if env_extra:
        env.update(env_extra)
    return subprocess.run(
        [sys.executable, "-m", "anan_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
