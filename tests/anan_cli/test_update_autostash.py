...
def test_stash_local_changes_if_needed_returns_specific_stash_commit(monkeypatch, tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append((cmd, kwargs))
        if cmd[-2:] == ["status", "--porcelain"]:
            return SimpleNamespace(stdout=" M anan_cli/main.py\n?? notes.txt\n", returncode=0)
...
def test_restore_stashed_changes_always_resets_on_conflict(monkeypatch, tmp_path, capsys):
    ...
    Leaving conflict markers in source files makes anan unrunnable (SyntaxError).
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd[1:3] == ["diff", "--name-only"]:
            return SimpleNamespace(stdout="anan_cli/main.py\n", stderr="", returncode=0)
    ...
    assert "anan_cli/main.py" in out
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(anan_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
