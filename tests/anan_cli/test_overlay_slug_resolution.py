...
_PROVIDER_MODELS and config.yaml use anan IDs ("copilot").  The slug
...
def test_copilot_uses_anan_slug():
...
def test_kimi_for_coding_overlay_uses_anan_slug():
...
def test_kilo_overlay_uses_anan_slug():
...
def test_mapped_provider_credential_pool_visibility(monkeypatch):
    ...
    monkeypatch.setattr(
        "anan_cli.auth._load_auth_store",
