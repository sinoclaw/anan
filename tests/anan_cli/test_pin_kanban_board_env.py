...
`anan kanban …`) inherit the same board the in-process kanban tools resolve.
Without this, a concurrent `anan kanban boards switch` from another session
...
def test_pin_writes_resolved_board_when_env_unset(monkeypatch):
    main_mod = importlib.import_module("anan_cli.main")

    import anan_cli.kanban_db as kdb
