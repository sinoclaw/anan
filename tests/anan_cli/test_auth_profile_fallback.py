...
When ``ANAN_HOME`` points to a named profile, ``read_credential_pool()``
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (ANAN_HOME == global root), no fallback path runs.
    ...
    # to the ANAN_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    anan_home = tmp_path / "classic"
    anan_home.mkdir()
    monkeypatch.setenv("ANAN_HOME", str(anan_home))

    _write(anan_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from anan_cli.auth import read_credential_pool, write_credential_pool
