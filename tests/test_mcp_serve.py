"""
Tests for mcp_serve — Anan MCP server.
...
def _isolate_anan_home(tmp_path, monkeypatch):
    """Redirect ANAN_HOME to a temp directory."""
    monkeypatch.setenv("ANAN_HOME", str(tmp_path))
    try:
        import anan_constants
        monkeypatch.setattr(anan_constants, "get_anan_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking anan_state schema."""
